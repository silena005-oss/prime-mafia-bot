...
async function proveritStartPlatnoyIgry(igra, kod) {
    ...
    return sBillingLockom(igra.klub_id, async () => {
        // Pascal / Prime Mafia — свои клубы, без платного лимита
        const { data: klubRow } = await supabase
            .from('kluby')
            .select('nazvaniye, nastroyki')
            .eq('id', igra.klub_id)
            .maybeSingle();
        if (etoDevKlubPoNazvaniyu(klubRow?.nazvaniye) || klubRow?.nastroyki?.bez_limita_tarifa) {
            igra._slot_oplaty = true;
            return { ok: true, tip: 'dev', klub_id: igra.klub_id };
        }

        const nastroyki = klubRow?.nastroyki
            ? { ...(klubRow.nastroyki || {}) }
            : await poluchitNastroykiKlubaBilling(igra.klub_id);
        const st = raschetStatusaTarifa(nastroyki);
...
/** Свои клубы разработчика — без тестового лимита, полный Network. */
function etoDevKlubPoNazvaniyu(nazvaniye) {
    const name = String(nazvaniye || '').toLowerCase();
    return /pascal|паскал|prime\s*mafia|прайм\s*мафия/.test(name);
}
