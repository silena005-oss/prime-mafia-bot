...
const { urlTelegramFile, poluchitMediaIzKasha, sohranitMediaVKesh } = require('./bezopasnost');
...
async function otsylkaAvatara(bot, file_id, res) {
    ...
    try {
        const cached = poluchitMediaIzKasha(file_id);
        if (cached) {
            res.writeHead(200, {
        ...
        const token = bot.token || process.env.TELEGRAM_TOKEN;
        ...
        sohranitMediaVKesh(file_id, contentType, buf);
        res.writeHead(200, {
