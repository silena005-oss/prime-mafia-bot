...
/** Короткий текст победы для чата: мирные — ники, мафия — команда и ники */
function tekstPobedyDlyaChata(igra, kod) {
    const pob = igra?.pobeditel;
    const igroki = igra?.igroki || [];
    let t = '🏁 *Итог игры*';
    if (kod) t += ' №' + kod;
    if (igra?.nomer_igry) t += ' · вечер №' + igra.nomer_igry;
    t += '\n';
    if (igra?.klub_nazvaniye) t += '🎴 ' + md(igra.klub_nazvaniye) + '\n';
    t += '\n';

    if (pob === 'mirnye') {
        const winners = igroki.filter(i => !isMafiaRole(i.rol) && i.rol !== 'Маньяк');
        const names = winners.map(i => md(i.name)).join(', ');
        t += '🟢 *Победили мирные:*\n';
        t += names || '—';
        t += '\n';
    } else if (pob === 'mafiya') {
        const maf = igroki.filter(i => isMafiaRole(i.rol));
        t += '🔴 *Победила команда мафии:*\n';
        if (maf.length) {
            maf.forEach(i => { t += '• *' + md(i.name) + '* — ' + i.rol + '\n'; });
        } else {
            t += '—\n';
        }
    } else if (pob === 'manyak') {
        const m = igroki.filter(i => i.rol === 'Маньяк');
        t += '🎯 *Победил маньяк:* ' + (m.map(i => md(i.name)).join(', ') || '—') + '\n';
    } else {
        t += 'Победитель не указан.\n';
    }
...
async function otpravitItogVGruppuKluba(bot, klub_id, text) {
    ...
    try {
        // Без превью ссылок; protect_content — нельзя переслать с атрибуцией.
        // Чтобы сообщение шло от имени группы (без имени бота) — сделай бота анонимным админом в чате.
        await bot.sendMessage(grp.chat_id, text, {
            parse_mode: 'Markdown',
