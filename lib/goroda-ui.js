...
/** Быстрый выбор — показываются первым рядом, если город есть в базе */
const POPULYARNYE_GORODA = {
    'Россия': ['Москва', 'Санкт-Петербург', 'Сочи', 'Казань', 'Екатеринбург', 'Новосибирск', 'Краснодар'],
    'Беларусь': ['Минск'],
...
function poiskGorodov(goroda, query, limit = 15) {
    ...
    return (goroda || [])
        ...
        .sort((a, b) => {
            const an = String(a.nazvaniye || '').toLowerCase();
            const bn = String(b.nazvaniye || '').toLowerCase();
            const aStart = an.startsWith(f) ? 0 : 1;
            const bStart = bn.startsWith(f) ? 0 : 1;
...
function populyarnyeGoroda(goroda, strana) {
    const names = POPULYARNYE_GORODA[strana] || [];
    const byName = Object.fromEntries((goroda || []).map(g => [g.nazvaniye, g]));
    return names.map(n => byName[n]).filter(Boolean);
}

function postroитьKlavPopulyarnye(goroda, strana, mode) {
    const p = prefiksy(mode);
    const pop = populyarnyeGoroda(goroda, strana);
    if (!pop.length) return [];
    const knopki = [];
    for (let i = 0; i < pop.length; i += 2) {
        const row = [{ text: '⭐ ' + pop[i].nazvaniye, callback_data: p.select + pop[i].id }];
        if (pop[i + 1]) row.push({ text: '⭐ ' + pop[i + 1].nazvaniye, callback_data: p.select + pop[i + 1].id });
        knopki.push(row);
    }
    return knopki;
}
...
function postroитьKlavAlfavit(goroda, mode, kod, strana) {
    const p = prefiksy(mode);
    const knopki = postroитьKlavPopulyarnye(goroda, strana, mode);
...
function tekstVyboraGoroda(strana, podzagolovok) {
    ...
    if (strana) t += ' (' + strana + ')';
    t += '\n\n';
    t += podzagolovok || '⭐ — популярные города\n🔤 — буква алфавита\n✍️ — напиши часть названия (*Моск*, *Соч*, *Пит*…)';
    return t;
}
