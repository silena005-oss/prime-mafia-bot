...
async function poluchitStatistikuPobedVechera(supabase, klub_id, data_igry) {
    // Берём завершённые игры клуба за сутки + запас до утра (вечер может закончиться после полуночи)
    const next = new Date(data_igry + 'T12:00:00');
    ...
    const { data: rows } = await supabase
        ...
        .lte('obnovlena_v', nextStr + 'T12:00:00');
    const stats = { mirnye: 0, mafiya: 0, manyak: 0, vsego: 0 };
    for (const row of rows || []) {
        const n = typeof row.nastroyki === 'string' ? JSON.parse(row.nastroyki || '{}') : (row.nastroyki || {});
        const gameDate = n.data_igry || (row.obnovlena_v ? String(row.obnovlena_v).slice(0, 10) : '');
        if (gameDate !== data_igry) continue;
        const p = n.pobeditel;
...
function formatStatistikuPobed(stats) {
    ...
    return [
        '📊 *Победы за вечер* (' + stats.vsego + ' ' + (stats.vsego === 1 ? 'игра' : stats.vsego < 5 ? 'игры' : 'игр') + '):',
        '🟢 Мирные — *' + stats.mirnye + '*',
        '🔴 Мафия — *' + stats.mafiya + '*',
        '🎯 Маньяк — *' + stats.manyak + '*'
    ].join('\n');
...
function formatLideraVechera(reyting) {
    if (!reyting?.length) return '_Лидера вечера пока нет — нет очков._';
    const top = reyting[0];
    let t = '⭐ *Лучший игрок вечера (по очкам):*\n';
    t += '1. *' + md(top.name || '?') + '* — *' + top.pts + '* очк.';
    if (top.games) t += ' (' + top.games + ' ' + (top.games === 1 ? 'игра' : 'игр') + ')';
    if (reyting.length > 1) {
        t += '\n_2–3 места:_\n';
        reyting.slice(1, 3).forEach((p, i) => {
            t += (i + 2) + '. ' + md(p.name || '?') + ' — ' + p.pts + '\n';
        });
    }
    return t.trim();
}
...
async function sohranitReytingVechera(supabase, klub_id, data_igry, reyting) {
    ...
    const { error } = await supabase
        ...
        .upsert(payload, { onConflict: 'klub_id,data_igry' });
