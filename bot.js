...
function shagIntroNochi(igra) {
    ...
    if (intro.phase === 'mirny') {
        ...
        return {
            ...
            candidates,
            can_confirm_all: candidates.length > 0 && candidates.length === remaining
        };
    }
    const roles = poryadokRoleyDlyaNochi(igra);
...
function metaHostaMiniApp(igra, kod) {
    ...
    const nazn = (igra.naznacheny_golos || []).map(n => {
        ...
        if (!p) return null;
        const immune = estImmunitetOtGolosovaniya(p, igra);
        return {
            nomer: p.nomer,
            name: p.name,
            votes: immune ? 0 : ((igra.golosa_dnya || {})[n] ?? null),
            immunity: immune
        };
    }).filter(Boolean);
    ...
    return {
        ...
        // Список на голосование можно править всегда в этих фазах — в т.ч. добавить пропущенного на чужой речи
        can_edit_nominees: ['den', 'znakomstvo', 'opravdanie', 'golosovanie'].includes(igra.faza),
        can_add_nominee: ['den', 'znakomstvo'].includes(igra.faza) &&
            kandidatyDobavitNaGolos(igra).length > 0,
        ...
        nominees: nominirovannyePoPoryadku(igra).map(i => ({
            nomer: i.nomer,
            name: i.name,
            immunity: estImmunitetOtGolosovaniya(i, igra)
        })),
        ...
        // День: после ночи, после интро (пропуск представления) или после круга знакомства
        can_start_day: igra.roli_razdany && !gov && !intro && (
            pickFaza === 'den' ||
            pickFaza === 'znakomstvo' ||
            (igra.faza === 'znakomstvo' && !!igra._krug_zavershen)
        ),
        intro_assigned: intro?.phase === 'roles'
            ? (igroki || []).filter(i => i.rol && i.rol !== 'Мирный').map(i => ({
                nomer: i.nomer, name: i.name, rol: i.rol
            }))
            : [],
...
async function sostoyanieMiniApp(user, opts = {}) {
    ...
    if (!roles.registered && !roles.isOwner && !isAdmin(telegram_id)) {
        return {
            user: {
                telegram_id,
                ...
                is_owner: false,
                is_host: false,
                can_manage_evening: false,
                can_import_rating: false,
                birthday: null,
                avatar_url: null
            },
            themes: [],
            clubs: [],
            selected_klub_id: null,
            evening: null,
            rating: { klub_id: null, enabled: false, my: null, top: [], best_game: null, role_stats: [] },
...
/** Незарег: только регистрация /start /help /unsubscribe. Остальное — приглашение. */
const CALLBACK_BEZ_REG = new Set([
    'pomoshch', 'scr_m', 'scr_x'
]);
function etoCallbackBezRegistracii(data) {
    const d = String(data || '');
    if (CALLBACK_BEZ_REG.has(d)) return true;
    if (d.startsWith('reg_') || d.startsWith('soglasie') || d.startsWith('club_') || d.startsWith('invite_')) return true;
    if (d.startsWith('anketa_') || d.startsWith('gorod_') || d.startsWith('strana_')) return true;
    if (d === 'nachat_registraciyu' || d === 'otmena_reg') return true;
    return false;
}

async function zablokirovatNezaregistrirovannogo(chatId, tg_id) {
    const roles = await poluchitRoliPolzovatelya(tg_id);
    if (roles.registered || isAdmin(tg_id)) return false;
    if (ozhidanie_registracii[tg_id]) return false;
    await napravitNezaregistrirovannogoNaInvite(chatId, tg_id, {});
    return true;
}
...
const sostavy = {
    ...
    21: ['Дон', 'Эскортница', 'Мафия', 'Мафия', 'Мафия', 'Мафия', 'Мафия', 'Шериф', 'Доктор', 'Стрелок', 'Камикадзе', 'Бессмертный', 'Шахид', 'Маньяк', 'Мирный', 'Мирный', 'Мирный', 'Мирный', 'Мирный', 'Мирный', 'Мирный']
};
...
async function otpravitDruzyaVoronkuPosleIgry(igra) {
    ...
    const tekst =
        ...
        'Можно получать анонсы игр клубов в *твоём городе* — с согласия, отписка: /unsubscribe.\n\n' +
...
/** Выбор кто за столом в эту игру: tg_id → { klub_id, selected: Set<number> } (индексы пула вечера) */
const vecherStolPick = {};

/** После раннего answerCallbackQuery show_alert молчит — пишем в чат. */
async function soobshitPosleKnopki(chatId, text) {
    if (!chatId || text == null || text === '') return;
    await bot.sendMessage(chatId, String(text).slice(0, 3500)).catch(() => {});
}

/** Кнопки по N в ряд (для 20 мест — не упираться в лимит рядов Telegram). */
function ryadyKnopokPoN(knopkiOdnoi, n = 2) {
    const rows = [];
    const list = knopkiOdnoi || [];
...
async function sohranit_igru(kod) {
    ...
    try {
        const nastroyki = {
            ...
            _posle_golosa: igra._posle_golosa || null
        };
        igra._nastroyki = nastroyki;
        const data = {
            kod,
        ...
        (rows || []).forEach(row => {
            if (!igry[row.kod]) {
                ...
                igry[row.kod] = {
                    ...
                    _posle_golosa: nastroyki._posle_golosa || null,
...
bot.on('message', async function(msg) {
    ...
    // Незарег: только регистрация и служебные команды
    const textLowEarly = text.toLowerCase();
    const sluzhebnoe = text.startsWith('/') ||
        textLowEarly === 'стоп' || textLowEarly === 'stop' ||
        textLowEarly === 'отписаться' || textLowEarly === 'unsubscribe' ||
        textLowEarly === 'подписаться' || textLowEarly === 'subscribe';
    if (!sluzhebnoe && !ozhidanie_registracii[tg_id] && !msg.web_app_data?.data) {
        if (await zablokirovatNezaregistrirovannogo(chatId, tg_id)) return;
    }
    ...
    if (fwdChat && ['group', 'supergroup', 'channel'].includes(fwdChat.type) && ozhidanie_registracii[tg_id]?.shag === 'privyazat_gruppu_kluba') {
        ...
        bot.sendMessage(chatId,
            ...
            '🔒 Чтобы итоги шли *от имени группы* (без имени бота): в группе → Админы → бот → включи *«Анонимность»* / Remain anonymous.',
            { parse_mode: 'Markdown' }
        );
        return;
    }
    ...
    if (sostoyanie[tg_id]?.startsWith('vecher_add_')) {
        ...
        if (!rez.ok) {
            const msg = rez.error === 'already'
                ? '❌ Эти ники уже в составе.\n\nЧтобы *заменить* уехавшего: кнопка «🔄 Заменить игрока» или `4 Снеговик`.'
                : '❌ Отправь ник(и) новых игроков.\n\nЗамена места: кнопка «🔄 Заменить игрока».';
            bot.sendMessage(chatId, msg, { parse_mode: 'Markdown' });
            return;
        }
        delete sostoyanie[tg_id];
        let zag = '✅ ';
        if (rez.replaced?.length) {
            zag += rez.replaced.map(r => '№' + r.seat + ': ' + r.from + ' → ' + r.to).join('; ');
        }
        if (rez.added?.length) {
            zag += (rez.replaced?.length ? '. ' : '') + 'Добавлены: ' + rez.added.join(', ');
        }
        zag += '. Всего: ' + rez.count + '.';
        await posleIzmeneniyaSpiskaVechera(chatId, tg_id, klub_id, zag);
        return;
    }

    if (sostoyanie[tg_id]?.startsWith('vecher_repnick_')) {
        const rest = sostoyanie[tg_id].replace('vecher_repnick_', '');
        const last = rest.lastIndexOf('_');
        const klub_id = rest.slice(0, last);
        const seat = parseInt(rest.slice(last + 1), 10);
        const nick = ochistitNikIzSpiska(text) || String(text || '').trim();
        if (!nick || !Number.isFinite(seat)) {
            bot.sendMessage(chatId, '❌ Пришли новый ник для места №' + seat + '.');
            return;
        }
        const rez = await dobavitIgrokovVSpisokVechera(klub_id, seat + ' ' + nick);
        if (!rez.ok) {
            bot.sendMessage(chatId, '❌ Не удалось заменить. Попробуй ещё раз или «🗑 Удалить» + «➕ Добавить».');
            return;
        }
        delete sostoyanie[tg_id];
        const r = rez.replaced?.[0];
        const zag = r
            ? '✅ Заменил №' + r.seat + ': ' + r.from + ' → *' + r.to + '*'
            : '✅ Место №' + seat + ' обновлено';
        await posleIzmeneniyaSpiskaVechera(chatId, tg_id, klub_id, zag);
        return;
    }
    ...
    if (sostoyanie[tg_id]?.startsWith('vecher_remove_')) {
        ...
        if (!rez.ok) {
            const msg = rez.error === 'not_found'
                ? '❌ Не нашёл таких в составе. Удобнее: «🗑 Удалить игрока» и нажать место.'
                : '❌ Отправь номер или ник, либо нажми «🗑 Удалить игрока».';
            bot.sendMessage(chatId, msg);
            return;
        }
        delete sostoyanie[tg_id];
        await posleIzmeneniyaSpiskaVechera(chatId, tg_id, klub_id,
            '✅ Убрано: *' + rez.removed + '*. Осталось: *' + rez.count + '*.');
        return;
    }
    ...
    if (sostoyanie[tg_id]?.startsWith('golos_count_')) {
        ...
        const igrok_chk = igra_gc.igroki.find(i => i.nomer === nomer_gc);
        if (estImmunitetOtGolosovaniya(igrok_chk, igra_gc)) {
            delete sostoyanie[tg_id];
            bot.sendMessage(chatId, '🛡 У игрока иммунитет — голоса за него не вводят.');
            return;
        }
    ...
    if (sostoyanie[tg_id]?.startsWith('perviy_hod_')) {
        ...
        const poisk_ph = naytiIgrokaPoVvoduDetalno(igra_ph, text);
        if (poisk_ph.error === 'ambiguous') {
            const list = (poisk_ph.matches || []).map(i => '№' + i.nomer + ' *' + md(i.name) + '*').join('\n');
            bot.sendMessage(chatId, '❓ Нашлось несколько:\n' + list + '\n\nУточни *№место* или полный ник.', { parse_mode: 'Markdown' });
            return;
        }
        const igrok_ph = poisk_ph.igrok;
        if (!igrok_ph || igrok_ph.status !== 'v_igre') {
            const hints = (poisk_ph.hints || []).map(i => '№' + i.nomer + ' ' + i.name).join(', ');
            const sostav = (igra_ph.igroki || [])
                .filter(i => i.status === 'v_igre')
                .map(i => '№' + i.nomer + ' ' + i.name)
                .join('\n');
            bot.sendMessage(chatId,
                '❌ Не нашёл такого игрока.' + (hints ? ' Похоже: ' + hints + '.' : '') +
                '\n\n*За столом:*\n' + sostav + '\n\nОтправь *номер*, *ник* или `авто`.',
                { parse_mode: 'Markdown' }
            );
            return;
        }
    ...
    if (sostoyanie[tg_id]?.startsWith('noch_znakomstvo_')) {
        ...
        // Вставка всего состава столбиком на шаге роли — частая ошибка («ник есть в списке, а бот не находит»)
        const nikiVzglyad = razobratSpisokNikov(text);
        if (nikiVzglyad.length > 1 || String(text || '').includes('\n')) {
            const bezRoli = (igra_nz.igroki || [])
                .filter(i => i.status === 'v_igre' && !i.rol)
                .sort((a, b) => a.nomer - b.nomer);
            let msg = '❌ Сейчас нужен *один* игрок для роли *' + rol_nz + '*.\n\n';
            msg += 'Не присылай весь список заново — состав уже в игре.\n';
            msg += 'Нажми кнопку места ниже или отправь *№места* / один ник.\n\n';
            if (bezRoli.length) {
                msg += '*Без роли:*\n' + bezRoli.map(i => '• №' + i.nomer + ' ' + md(i.name)).join('\n');
            }
            await otpravitOshibkuNochiZnakomstva(chatId, igra_nz, kod_nz, idx_nz, msg);
            return;
        }
        ...
        if (!igrok_nz) {
            if ((igra_nz.igroki || []).length >= igra_nz.kolichestvo) {
                const bezRoli = (igra_nz.igroki || [])
                    .filter(i => i.status === 'v_igre' && !i.rol)
                    .sort((a, b) => a.nomer - b.nomer);
                const hints = (poisk.hints || []).map(i => '№' + i.nomer + ' ' + i.name).join(', ');
                let msg = '❌ Не нашёл *' + md(text.trim()) + '* в составе этой игры.\n\n';
                if (bezRoli.length) {
                    msg += '*Сейчас без роли (нажми кнопку на шаге ниже):*\n';
                    msg += bezRoli.map(i => '• №' + i.nomer + ' ' + md(i.name)).join('\n');
                    msg += '\n\n';
                }
                if (hints) msg += 'Похоже: ' + hints + '\n\n';
                msg += '_Ник мог быть записан иначе — удобнее выбрать место кнопкой или ввести *№места*._';
                await otpravitOshibkuNochiZnakomstva(chatId, igra_nz, kod_nz, idx_nz, msg);
                return;
            }
            igrok_nz = {
    ...
    if (ozhidanie_registracii[tg_id]?.shag === 'anketa_klub') {
        ...
        if (pole && (pole.tip === 'text' || pole.tip === 'text_skip')) {
            ...
            // Кнопочный шаг: игрок написал текст — подсказываем выбрать кнопкой
            bot.sendMessage(chatId, '👆 На этом шаге выбери вариант кнопкой под вопросом (или «⏭ Пропустить анкету»).');
        }
        return;
    }
...
function etoIstochnikAvtoHoda(source) {
    return source === 'pass' || source === 'timer' || source === 'miniapp';
}

function nedavnoPereshliNaTekushchego(igra, now = Date.now()) {
    return !!(igra && igra._last_advanced_at && (now - igra._last_advanced_at) < 1600);
}
...
function otkrytOverlayTaymera(igra, tip) {
    ...
    if (fazaKRuchiRechi(igra.faza) && igra.tekushchiy_nomer && !igra._krug_zavershen) {
        ...
        igra._picker_type = tip || 'overlay';
    }
}

function knopkaNazadKProcessu(igra, kod) {
    if (!igra) return 'panel_' + kod;
    if (igra.faza === 'golosovanie') return 'faza_golosovanie_' + kod;
    if (igra._posle_golosa) return 'posle_golosa_' + kod;
    if (fazaKRuchiRechi(igra.faza) && igra.tekushchiy_nomer && !igra._krug_zavershen) return 'timer_back_' + kod;
    if (igra.faza === 'opravdanie') return 'faza_opravdanie_' + kod;
    if (igra.faza === 'noch') return 'noch_panel_' + kod;
    return 'panel_' + kod;
}

/** После голосования: выбывший → лучший ход (день 1) → ночь. Не теряем этот шаг. */
async function pokazatPosleGolosovaniya(chatId, messageId, kod, nomerVybyv, opts = {}) {
    const igra = igry[kod];
    if (!igra || !chatId) return;
    const igrok = nomerVybyv != null ? igra.igroki.find(i => i.nomer === nomerVybyv) : null;
    const needLh = !!(igrok && (igra.den || 1) === 1 && mozhetBytLuchshiyHod(igrok));
    igra._posle_golosa = {
        nomer: nomerVybyv == null ? null : nomerVybyv,
        need_lh: needLh,
        next: 'noch',
        shahid: opts.shahidEffect || null,
        log: opts.logMsg || null
    };
    await sohranit_igru(kod);

    let t = '\uD83D\uDDF3 *Итог голосования*\n\n';
    if (!igrok) {
        t += '\u2705 Никто не выбыл.\n';
    } else {
        t += '\uD83D\uDC80 Выбывает \u2116' + igrok.nomer + ' *' + igrok.name + '*';
        if (igrok.rol) t += ' — ' + igrok.rol;
        t += '\n';
    }
    if (opts.shahidEffect) t += '\n' + opts.shahidEffect + '\n';
    if (needLh) {
        t += '\nДальше: *лучший ход* (если не мафия), затем *ночь ' + (igra.den || 1) + '*.';
    } else {
        t += '\nДальше: *ночь ' + (igra.den || 1) + '*.';
    }

    const knopki = [];
    if (needLh) {
        knopki.push([{
            text: '\uD83C\uDFC6 Лучший ход \u2116' + igrok.nomer,
            callback_data: 'lh_start_' + kod + '_' + igrok.nomer + '_den1_noch'
        }]);
        knopki.push([{ text: '\u23ED\uFE0F Пропустить лучший ход → ночь', callback_data: 'faza_noch_' + kod }]);
    } else {
        knopki.push([{ text: '\uD83C\uDF19 К ночи', callback_data: 'faza_noch_' + kod }]);
    }
    ...
    try {
        ...
        await bot.sendMessage(chatId, t, payload).catch(() => {});
    }
}
...
async function vernutPanelTaymera(igra, kod, chatId, messageId, opts = {}) {
    ...
    try {
        ...
        if (opts.restartTimer !== false && igra.tekushchiy_nomer && !igra._krug_zavershen && !timerZapushchen) {
            perezapustitTaymerRechi(igra, kod, chat_id, message_id);
        }
        return true;
    } catch (_) {
        return false;
...
function naznachitImmunitetIgroku(igra, nomer) {
    ...
    if (igrok) {
        igrok.immunitet_posle_nochi = true;
        delete igrok.immunitet_snyat_vedushchim;
    }
}

/** Первая смерть игры (ночь 1 или первое голосование дня 1) → щит на следующую игру вечера. */
async function otmetitPervuyuSmertDlyaImmuniteta(igra, igrok, prichina) {
    if (!igra || !igrok || igra._immunitet_pervoy_smerti_naznachen) return;
    if ((igra.den || 1) !== 1) return;
    if (prichina !== 'noch1' && prichina !== 'golos1') return;
    igra._immunitet_pervoy_smerti_naznachen = true;
    const info = {
        igrok_id: igrok.igrok_id || null,
        telegram_id: igrok.telegram_id || null,
        name: igrok.name || '',
        prichina,
        kod: null
    };
    igra._immunitet_pervoy_smerti = info;
    if (igra.klub_id) {
        try {
            await obnovitNastroykiVecheraKluba(igra.klub_id, {
                vecher_immunitet_next: {
                    ...info,
                    sozdan: new Date().toISOString()
                }
            });
        } catch (e) {
            console.error('[immunitet first death]', e?.message || e);
        }
    }
}

async function primenitVecherImmunitetNextKIgre(igra) {
    if (!igra?.klub_id) return null;
    const nastroyki = await poluchitNastroykiVecheraKluba(igra.klub_id);
    const info = nastroyki?.vecher_immunitet_next;
    if (!info) return null;
    const match = (igra.igroki || []).find(i =>
        i.status === 'v_igre' && (
            (info.igrok_id && i.igrok_id === info.igrok_id) ||
            (info.telegram_id && i.telegram_id === info.telegram_id) ||
...
function sekundyTaymeraRechi(igra) {
    const nastroyki = igra?._nastroyki || {};
    if (igra?.faza === 'znakomstvo') return nastroyki.znakomstvo_sek || 15;
    if (igra?.faza === 'opravdanie') return nastroyki.opravdanie_sek || 30;
    return lichnoeVremyaSek(igra);
}

function perezapustitTaymerRechi(igra, kod, chatId, messageId) {
    if (!igra?.tekushchiy_nomer) return;
    const { chat_id, message_id } = idSoobshcheniyaTaymera(igra, chatId, messageId);
    const ostalos = igra.taymer_sekundy > 0 ? igra.taymer_sekundy : sekundyTaymeraRechi(igra);
    zapustitTaymer(chat_id || null, message_id || null, kod, ostalos);
}
...
function tekstPodskazkiPoiskaIgroka() {
    return '· *№12* / *#12* / *место 12* — номер за столом\n' +
        ...
        '· *Аня* — по нику (можно часть ника)\n' +
        '· *№3 Снеговик* — тоже ок, возьмём место или ник';
}

function normalizovatNickDlyaPoiska(text) {
    const LATIN_TO_CYR = {
        a: 'а', A: 'а', e: 'е', E: 'е', o: 'о', O: 'о', p: 'р', P: 'р',
        c: 'с', C: 'с', x: 'х', X: 'х', y: 'у', Y: 'у', k: 'к', K: 'к',
        m: 'м', M: 'м', t: 'т', T: 'т', h: 'н', H: 'н', b: 'в', B: 'в'
    };
    return String(text || '')
        .normalize('NFKC')
        .replace(/[\u200B-\u200D\uFEFF]/g, '')
        .replace(/ё/gi, 'е')
        .replace(/[aAeEoOpPcCxXyYkKmMtThHbB]/g, ch => LATIN_TO_CYR[ch] || ch)
        .replace(/\s+/g, ' ')
        .trim()
        .toLowerCase();
}
...
function razobratVvodIgroka(text) {
    ...
    const lower = normalizovatNickDlyaPoiska(raw);
    // «№3 Снеговик» / «3. Снеговик» / «место 3 Снеговик» → сначала место
    const seatWithNick = lower.match(/^(?:№|#|n|место|номер)?\s*(\d+)(?:[\.\):,\-\s]+.*)?$/);
    const seatExplicit = lower.match(/^(?:№|#|n|место|номер)\s*(\d+)$/);
    if (seatExplicit) return { kind: 'seat', seat: parseInt(seatExplicit[1], 10), raw };
    if (seatWithNick && /^(?:№|#|n|место|номер)/i.test(raw.trim())) {
        return { kind: 'seat', seat: parseInt(seatWithNick[1], 10), raw };
    }
    // «3 Снеговик» без префикса — пробуем как ник целиком; seat отдельно через kind number только для чистых цифр
    if (/^\d+$/.test(raw.trim())) return { kind: 'number', seat: parseInt(raw, 10), raw };
...
function naytiIgrokaPoNicku(igra, query) {
    const vvod = normalizovatNickDlyaPoiska(query);
    if (!vvod) return { igrok: null };
    const igroki = igra.igroki || [];
    const nickOf = (i) => normalizovatNickDlyaPoiska(i?.name);
    const exact = igroki.filter(i => nickOf(i) === vvod);
    if (exact.length === 1) return { igrok: exact[0] };
    if (exact.length > 1) return { igrok: null, error: 'ambiguous', matches: exact };

    // «№3 Снеговик» / «3. Снеговик» → ник без номера
    const bezNomera = vvod.replace(/^(?:№|#|n|место|номер)?\s*\d+[\.\):,\-\s]*/, '').trim();
    if (bezNomera && bezNomera !== vvod) {
        const byClean = igroki.filter(i => nickOf(i) === bezNomera);
        if (byClean.length === 1) return { igrok: byClean[0] };
        if (byClean.length > 1) return { igrok: null, error: 'ambiguous', matches: byClean };
    }

    const partial = igroki.filter(i => {
        const n = nickOf(i);
        return n.includes(vvod) || (bezNomera && n.includes(bezNomera)) || vvod.includes(n);
    });
    if (partial.length === 1) return { igrok: partial[0] };
    if (partial.length > 1) return { igrok: null, error: 'ambiguous', matches: partial };

    // Ближайшие подсказки (префикс / общее начало)
    const hints = igroki
        .filter(i => {
            const n = nickOf(i);
            const q = bezNomera || vvod;
            return n.startsWith(q.slice(0, Math.min(3, q.length))) || q.startsWith(n.slice(0, Math.min(3, n.length)));
        })
        .slice(0, 5);
    return { igrok: null, error: 'not_found', hints };
}
...
function knopkiShagaNochiZnakomstva(igra, kod, idx) {
    const knopki = [];
    const bezRoli = (igra.igroki || [])
        .filter(i => i.status === 'v_igre' && mirnySlotBezRoli(i.rol))
        .sort((a, b) => a.nomer - b.nomer);
    // Кнопки мест — чтобы не гадать ник («Красавчик» vs как записан в составе)
    const seatBtns = bezRoli.map(i => ({
        text: ('№' + i.nomer + ' ' + String(i.name || '')).slice(0, 32),
        callback_data: 'nz_seat_' + kod + '_' + i.nomer
    }));
    knopki.push(...ryadyKnopokPoN(seatBtns, 2));
...
function tekstShagaNochiZnakomstva(igra, kod, idx) {
    ...
    t += 'Роль: *' + label + '*\n\n';
    if (bezRoli.length) {
        t += '*Без роли — нажми кнопку или введи №/ник:*\n';
        t += bezRoli.map(i => '• \u2116' + i.nomer + ' ' + md(i.name)).join('\n');
        t += '\n\n';
    } else {
        t += '_Все места уже с ролями — исправь внесённые, если ошибка._\n\n';
    }
    t += 'Можно отправить номер или ник текстом.\n';
    t += tekstPodskazkiPoiskaIgroka() + '\n\n';
    t += '_Мирных на этом шаге не вводим — их подтвердим после всех активных ролей._';
    return t;
}
...
async function zavershitNochZnakomstva(chatId, kod, opts = {}) {
    ...
    try {
        ...
        // Не затираем пул вечера (20 чел.), если за столом только часть
        if (igra.klub_id) {
            const pool = await poluchitSpisokVecheraKluba(igra.klub_id);
            const zaStolom = (igra.igroki || []).length;
            if (!pool?.length || pool.length <= zaStolom) {
                await sohranitSpisokVecheraKluba(igra.klub_id, igra.igroki);
            }
        }
        await finalizirovatImmunitetyPriStarteIgry(igra, kod);
        await sohranit_igru(kod);
...
async function knopkiPosleItogaIgry(kod, klub_id, opts = {}) {
    if (opts.druzya) {
        return {
            inline_keyboard: [
                [{ text: '🎮 Ещё игра с друзьями', callback_data: 'druzya_menu' }],
    ...
    if (klub_id) {
        try {
            ...
            if (dannye.spisok?.length && !dannye.zavershen) {
                const nextN = await predlozhennyyNomerIgryVechera(klub_id, dataIgrovoegoVechera());
                rows.push([{ text: '▶️ Следующая игра вечера (№' + nextN + ')', callback_data: 'vecher_sozdat_igru_' + klub_id }]);
                rows.push([{ text: '🌙 Игровой вечер', callback_data: 'vecher_klub_' + klub_id }]);
            }
        } catch (e) {
            console.error('[knopkiPosleItogaIgry vecher]', e?.message || e);
        }
    }
    rows.push(
        [{ text: '🎁 Добавить бонусы', callback_data: 'bonusy_' + kod }],
        [{ text: '🎲 Новая игра с нуля', callback_data: 'sozdat_igru' }],
        [{ text: '🏠 В меню', callback_data: 'menu_vedushchego' }]
    );
    return { inline_keyboard: rows };
}
...
function igrokiBezRoliDlyaMirnyh(igra) {
    return (igra.igroki || []).filter(i => i.status === 'v_igre' && mirnySlotBezRoli(i.rol));
}
...
function tekstVvodaSpiskaMirnyh(igra, kod) {
    ...
    const sRolyami = (igra.igroki || []).filter(i => i.rol && i.rol !== 'Мирный');
    const mirnye = (igra.igroki || [])
        .filter(i => i.status === 'v_igre' && uzheVnesenMirny(i.rol))
        .sort((a, b) => a.nomer - b.nomer);
    ...
    if (sRolyami.length) {
        t += ':\n' + sRolyami.map(i => '• \u2116' + i.nomer + ' ' + md(i.name) + ' — ' + i.rol).join('\n');
    }
    t += '\n\n';

    if (bezRoli.length === 0) {
        if (mirnye.length) {
            t += '*Мирные* (*' + mirnye.length + '*):\n';
            t += mirnye.map(i => '• \u2116' + i.nomer + ' ' + md(i.name)).join('\n');
            t += '\n\n';
        }
        t += 'Все роли уже назначены. Можно завершить ночь.';
        return t;
    }

    t += 'Остались без роли (*' + bezRoli.length + '*):\n';
    t += bezRoli.map(i => '• \u2116' + i.nomer + ' *' + md(i.name) + '*').join('\n');
    t += '\n\n';

    if (bezRoli.length === nuzhno) {
        t += 'Это *мирные жители*?\n\n';
        t += '_Нажми «Да» — всем им поставим роль Мирный. Или «Редактировать», если кто-то с активной ролью._';
    } else {
        t += '\u26A0\uFE0F По составу мирных нужно *' + nuzhno + '*, без роли сейчас *' + bezRoli.length + '*.\n';
        t += 'Проверь активные роли через «Редактировать».';
    }
    return t;
}

function knopkiMirnyhVvoda(igra, kod, rezhim) {
    const knopki = [];
    const bezRoli = igrokiBezRoliDlyaMirnyh(igra);
    const nuzhno = mirnyeOstalosVnesti(igra);
    ...
    if (rezhim === 'edit') {
        ...
        knopki.push([{ text: '\u270F\uFE0F Исправить активные роли', callback_data: 'nz_restart_' + kod }]);
        knopki.push([{ text: '\u2B05\uFE0F Назад к подтверждению', callback_data: 'mirny_podtv_' + kod }]);
        return { inline_keyboard: knopki };
    }

    if (bezRoli.length > 0 && bezRoli.length === nuzhno) {
...
async function podtverditVsehMirnyh(chatId, igra, kod) {
    const bezRoli = igrokiBezRoliDlyaMirnyh(igra);
    const nuzhno = mirnyeOstalosVnesti(igra);
...
async function soobshitMirnogoDobavlen(chatId, igra, kod, igrok, ostalos) {
    ...
    await bot.sendMessage(chatId, t, {
        parse_mode: 'Markdown',
        reply_markup: knopkiMirnyhVvoda(igra, kod, ostalos > 0 ? 'edit' : null)
    });
}
...
function parseVecherSeatCb(data, prefix) {
    const rest = String(data || '').replace(prefix, '');
    const last = rest.lastIndexOf('_');
    if (last < 0) return null;
    const klub_id = rest.slice(0, last);
    const nomer = parseInt(rest.slice(last + 1), 10);
    if (!klub_id || !Number.isFinite(nomer) || nomer < 1) return null;
    return { klub_id, nomer };
}

async function tekstSpiskaVecheraKorotkiy(klub_id) {
    const spisok = await poluchitSpisokVecheraKluba(klub_id);
    if (!spisok?.length) return '_Состав пуст._';
    return spisok.map((p, i) => (i + 1) + '. ' + md(p.name || 'Игрок')).join('\n');
}

async function pokazatVyborMestaVechera(chatId, messageId, klub_id, rezhim) {
    const spisok = await poluchitSpisokVecheraKluba(klub_id);
    if (!spisok?.length) {
        const t = '❌ Состав вечера пуст — сначала внеси игроков.';
        const knopki = [[{ text: '✍️ Внести состав', callback_data: 'vecher_vvod_' + klub_id }],
            [{ text: '⬅️ Назад', callback_data: 'vecher_klub_' + klub_id }]];
        if (messageId) {
            await bot.editMessageText(t, { chat_id: chatId, message_id: messageId, reply_markup: { inline_keyboard: knopki } }).catch(() => {});
        } else {
            await bot.sendMessage(chatId, t, { reply_markup: { inline_keyboard: knopki } });
        }
        return;
    }
    const del = rezhim === 'del';
    let t = del
        ? '🗑 *Удалить игрока*\n\nКто ушёл / встал и уехал? Нажми место:\n\n'
        : '🔄 *Заменить игрока*\n\nКого меняем за столом? Нажми место, затем пришли новый ник:\n\n';
    t += await tekstSpiskaVecheraKorotkiy(klub_id);
    const prefix = del ? 'vecher_delpick_' : 'vecher_reppick_';
    const seatBtns = spisok.map((p, i) => ({
        text: (del ? '🗑 ' : '🔄 ') + '№' + (i + 1) + ' ' + String(p.name || '').slice(0, 28),
        callback_data: prefix + klub_id + '_' + (i + 1)
    }));
    const knopki = ryadyKnopokPoN(seatBtns, 2);
    knopki.push([{ text: '⬅️ Назад', callback_data: 'vecher_klub_' + klub_id }]);
    const opts = { parse_mode: 'Markdown', reply_markup: { inline_keyboard: knopki } };
    if (messageId) {
        await bot.editMessageText(t, { chat_id: chatId, message_id: messageId, ...opts }).catch(() => bot.sendMessage(chatId, t, opts));
    } else {
        await bot.sendMessage(chatId, t, opts);
    }
}

async function posleIzmeneniyaSpiskaVechera(chatId, tg_id, klub_id, zagolovok) {
    const kodLobby = await obnovitLobbyVecheraIzSpiska(tg_id, klub_id);
    const { data: klub } = await supabase.from('kluby').select('id, nazvaniye, nastroyki').eq('id', klub_id).single();
    const spisokText = await tekstSpiskaVecheraKorotkiy(klub_id);
    const soobsh = await bot.sendMessage(chatId,
        zagolovok + '\n\n*Состав сейчас:*\n' + spisokText,
        { parse_mode: 'Markdown' }
    );
    if (kodLobby) {
        await pokazatSostavSleduyushcheyIgryVechera(chatId, soobsh.message_id, kodLobby);
    } else {
        await pokazatIgrovoyVecher(chatId, soobsh.message_id, klub || { id: klub_id, nazvaniye: '' }, tg_id);
    }
}

function knopkiPravkiSostavaVechera(klub_id) {
    return [
        [{ text: '➕ Добавить игрока', callback_data: 'vecher_add_' + klub_id }],
        [{ text: '🔄 Заменить игрока', callback_data: 'vecher_replace_' + klub_id }],
        [{ text: '🗑 Удалить игрока', callback_data: 'vecher_delete_' + klub_id }]
    ];
}

function poluchitIliSozdatVyborStola(tg_id, klub_id, spisokLen) {
    let st = vecherStolPick[tg_id];
    if (!st || String(st.klub_id) !== String(klub_id)) {
        st = {
            klub_id,
            selected: new Set(Array.from({ length: spisokLen }, (_, i) => i))
        };
        vecherStolPick[tg_id] = st;
    }
    // Убрать индексы вне текущего пула
    for (const idx of [...st.selected]) {
        if (idx < 0 || idx >= spisokLen) st.selected.delete(idx);
    }
    return st;
}

async function pokazatVyborStolaIgry(chatId, messageId, klub_id, tg_id) {
    const spisok = await poluchitSpisokVecheraKluba(klub_id);
    if (!spisok?.length) {
        await soobshitPosleKnopki(chatId, '❌ Сначала зафиксируй состав вечера.');
        return false;
    }
    const st = poluchitIliSozdatVyborStola(tg_id, klub_id, spisok.length);
    const nSel = st.selected.size;
    let t = '🎲 *Кто играет в эту игру?*\n\n';
    t += 'Пул вечера: *' + spisok.length + '*. Выбрано за стол: *' + nSel + '*.\n';
    t += '_Отметь тех, кто садится сейчас. Остальные остаются в пуле на следующие игры._\n';
    t += '_Состав ролей — на следующем шаге (от 8 игроков)._\n\n';
    spisok.forEach((p, i) => {
        const mark = st.selected.has(i) ? '✅' : '⬜';
        t += mark + ' №' + (i + 1) + ' ' + md(p.name || '') + '\n';
    });
    const seatBtns = spisok.map((p, i) => ({
        text: (st.selected.has(i) ? '✅' : '⬜') + ' №' + (i + 1) + ' ' + String(p.name || '').slice(0, 22),
        callback_data: 'vecher_stol_tog_' + klub_id + '_' + i
    }));
    const knopki = ryadyKnopokPoN(seatBtns, 2);
    knopki.push([
        { text: '✅ Все', callback_data: 'vecher_stol_all_' + klub_id },
        { text: '⬜ Снять', callback_data: 'vecher_stol_clr_' + klub_id }
    ]);
    knopki.push([{ text: '▶️ Дальше · ' + nSel + ' за столом', callback_data: 'vecher_stol_ok_' + klub_id }]);
    knopki.push([{ text: '⬅️ Назад', callback_data: 'vecher_klub_' + klub_id }]);
    const opts = { parse_mode: 'Markdown', reply_markup: { inline_keyboard: knopki } };
    if (messageId) {
        await bot.editMessageText(t, { chat_id: chatId, message_id: messageId, ...opts })
...
async function pereytiKPreviewIzVyborStola(chatId, messageId, klub_id, tg_id) {
    const spisok = await poluchitSpisokVecheraKluba(klub_id);
    if (!spisok?.length) {
        await soobshitPosleKnopki(chatId, '❌ Состав вечера пуст.');
        return false;
    }
    const st = poluchitIliSozdatVyborStola(tg_id, klub_id, spisok.length);
    const selected = [...st.selected].sort((a, b) => a - b).map(i => spisok[i]).filter(Boolean);
    const kolichestvo = selected.length;
    if (kolichestvo < 8) {
        await soobshitPosleKnopki(chatId, '❌ Нужно минимум 8 игроков за столом (сейчас ' + kolichestvo + ').');
        return false;
    }
    const { data: klub } = await supabase
        .from('kluby')
    ...
    if (!preview) {
        await soobshitPosleKnopki(chatId, '❌ Нет состава на ' + kolichestvo + ' игроков.');
        return false;
    }
    const previewId = sozdatPreviewSostava({
        ...
        kolichestvo,
        from_vecher: true,
        vecher_igroki: selected,
        nazad_callback: 'vecher_stol_' + klub_id
    });
...
/** «4 Снеговик» / «№4. Снеговик» / «место 4 — Снеговик» → замена места, не добавление в конец. */
function razobratStrokuZamenyMesta(line) {
    const raw = String(line || '').trim();
    if (!raw) return null;
    const m = raw.match(/^(?:№|#|n|место|номер)?\s*(\d+)\s*(?:[.):,\u2014\u2013–—\s-]+)\s*(.+)$/iu);
    if (!m) return null;
    const seat = parseInt(m[1], 10);
    let nick = String(m[2] || '').trim();
    // Не трогаем номер места повторно — только мусор вокруг ника
    nick = nick.replace(/^(?:№|#)\s*/u, '').trim();
    if (!Number.isFinite(seat) || seat < 1 || !nick || /^\d+$/.test(nick)) return null;
    return { seat, nick };
}
...
async function dobavitIgrokovVSpisokVechera(klub_id, text) {
    ...
    let current = Array.isArray(spisok) ? spisok.map(p => ({ ...p })) : [];
    const existing = new Set(current.map(p => normalizovatNickDlyaPoiska(p.name)));
    const replaced = [];
    const added = [];
    const skipped = [];

    for (const line of lines) {
        const zam = razobratStrokuZamenyMesta(line);
        if (zam) {
            if (zam.seat < 1) continue;
            // Дописать пустые места, если номер дальше конца списка
            while (current.length < zam.seat) {
                current.push({ name: 'Игрок', telegram_id: null, igrok_id: null });
            }
            const prev = current[zam.seat - 1];
            const prevNick = normalizovatNickDlyaPoiska(prev?.name);
            const nextNick = normalizovatNickDlyaPoiska(zam.nick);
            if (prevNick) existing.delete(prevNick);
            const igrok = {
                telegram_id: null,
                name: zam.nick,
                nomer: zam.seat,
            ...
            await privyazatIgrokaIzBazy({ klub_id, igroki: current }, igrok);
            current[zam.seat - 1] = {
                name: igrok.name,
                telegram_id: igrok.telegram_id || null,
                igrok_id: igrok.igrok_id || null
            };
            existing.add(normalizovatNickDlyaPoiska(igrok.name));
            replaced.push({
                seat: zam.seat,
                from: prev?.name || '—',
                to: igrok.name
        ...
        const nick = ochistitNikIzSpiska(line);
        if (!nick) continue;
        const key = normalizovatNickDlyaPoiska(nick);
        if (existing.has(key)) {
            skipped.push(nick);
            continue;
        }
        ...
        existing.add(normalizovatNickDlyaPoiska(igrok.name));
        added.push(igrok.name);
    }

    if (!replaced.length && !added.length) {
        return { ok: false, error: skipped.length ? 'already' : 'empty', skipped };
    }

    const igraTemp = {
        klub_id,
        igroki: current.map((p, idx) => ({
            telegram_id: p.telegram_id || null,
            name: p.name,
            nomer: idx + 1,
            status: 'v_igre',
    ...
    return {
        ok: true,
        count: igraTemp.igroki.length,
        replaced,
        added,
        skipped,
        spisok: igraTemp.igroki
    };
}
...
async function ubratIgrokovIzSpiskaVechera(klub_id, text) {
    ...
    for (const item of niki) {
        ...
        const idxNum = /^\d+$/.test(raw) ? parseInt(raw, 10) : NaN;
        const q = normalizovatNickDlyaPoiska(raw);
        current.forEach((p, idx) => {
            const name = normalizovatNickDlyaPoiska(p.name);
            if ((Number.isFinite(idxNum) && idx + 1 === idxNum) ||
                name === q ||
    ...
    await sohranitSpisokVecheraKluba(klub_id, igraTemp.igroki);
    return { ok: true, count: igraTemp.igroki.length, removed: remove.size, spisok: igraTemp.igroki };
}
...
/** Если уже создано лобби следующей игры вечера — подтянуть актуальный состав */
async function obnovitLobbyVecheraIzSpiska(telegram_id, klub_id) {
    if (!telegram_id || !klub_id) return null;
    const spisok = await poluchitSpisokVecheraKluba(klub_id);
    const lobby = Object.entries(igry).find(([kod, igra]) => {
        if (String(kod).startsWith('archive_') || String(kod).startsWith('preview_')) return false;
        return igra?.vedushchii_id === telegram_id &&
            String(igra.klub_id) === String(klub_id) &&
            !igra.roli_razdany;
    });
    if (!lobby) return null;
    const [kod, igra] = lobby;
    const lobbyLen = (igra.igroki || []).length;
    // Стол уже собран из части пула (напр. 10 из 20) — не раздуваем лобби всем вечером
    if (lobbyLen > 0 && spisok?.length > lobbyLen) {
        return kod;
    }
    if (!spisok?.length) {
        igra.kolichestvo = 0;
        ...
        igra.kolichestvo = spisok.length;
...
async function sohranitKastomnyySostavKluba(klub_id, kolichestvo, sostav) {
    if (!klub_id || !Array.isArray(sostav) || !sostav.length) return;
    try {
        const { data: klub } = await supabase.from('kluby').select('nastroyki').eq('id', klub_id).maybeSingle();
        const nastroyki = { ...(klub?.nastroyki || {}) };
        nastroyki.kastomnye_sostavy = {
            ...(nastroyki.kastomnye_sostavy || {}),
            [String(kolichestvo)]: [...sostav]
        };
        await sohranitNastroykiKlubaBilling(klub_id, nastroyki);
    } catch (e) {
        console.error('[sohranitKastomnyySostavKluba]', e?.message || e);
    }
}

async function sozdatIgryIzVechera(tg_id, klub_id, opts = {}) {
    const polnyy = await poluchitSpisokVecheraKluba(klub_id);
    if (!polnyy?.length) return null;
    const spisok = Array.isArray(opts.igroki) && opts.igroki.length
        ? normalizovatSpisokVechera(opts.igroki)
        : polnyy;
    ...
    // Подтянуть кастомный состав с прошлой игры вечера, если в клубе ещё не сохранён
    const kastom = igra._nastroyki?.kastomnye_sostavy?.[String(spisok.length)]
        || igra._nastroyki?.kastomnye_sostavy?.[spisok.length];
    if (Array.isArray(kastom) && kastom.length) {
        igra._sostav_custom = [...kastom];
    } else {
        const pred = Object.values(igry).find(g =>
            g && String(g.klub_id) === String(klub_id) &&
            Array.isArray(g._sostav_custom) && g._sostav_custom.length === spisok.length &&
            g.roli_razdany
        );
        if (pred) igra._sostav_custom = [...pred._sostav_custom];
    }
    await zapolnitIgruIzSpiskaVechera(igra, spisok);
    // Пул вечера не трогаем — за столом может быть только часть из 20
    await sohranit_igru(kod);
    return kod;
}
...
async function perehodPosleGolosovaniya(igra, kod, chatId, messageId, opts = {}) {
    ...
    if (chatId) {
        await pokazatPosleGolosovaniya(chatId, messageId, kod, nomerVybyv == null ? null : nomerVybyv, opts);
        const needLh = nomerVybyv != null
            && (igra.den || 1) === 1
            && mozhetBytLuchshiyHod(igra.igroki.find(i => i.nomer === nomerVybyv));
        return { eliminated: nomerVybyv, best_move: needLh };
    }
    const pobeditel = opredelitPobeditelya(igra);
    if (pobeditel) return { game_over: true, eliminated: nomerVybyv };
...
async function primeniRuchnoyItogGolosovaniya(igra, kod, nomerVybyv, chatId, messageId) {
    ...
    await otmetitPervuyuSmertDlyaImmuniteta(igra, igrok, 'golos1');
...
async function primeniItogGolosovaniyaMiniApp(igra, kod) {
    ...
    await otmetitPervuyuSmertDlyaImmuniteta(igra, vybyv, 'golos1');
...
async function miniAppVecherAction(tg_id, user, body) {
    ...
    if (sub === 'set_roster') {
        ...
        await sohranitSpisokVecheraKluba(klub_id, igraTemp.igroki);
        await obnovitLobbyVecheraIzSpiska(tg_id, klub_id);
    ...
    if (sub === 'add') {
        ...
        if (!rez.ok) {
            return {
                stay: true,
                message: rez.error === 'already'
                    ? 'Уже в составе. Замена места: «4 Снеговик»'
                    : 'Проверь ники. Замена: «4 НовыйНик», добавление: просто ник'
            };
        }
        await obnovitLobbyVecheraIzSpiska(tg_id, klub_id);
        let msg = 'В составе: ' + rez.count;
        if (rez.replaced?.length) {
            msg += '. Замены: ' + rez.replaced.map(r => '№' + r.seat + ' ' + r.from + '→' + r.to).join(', ');
        }
        if (rez.added?.length) msg += '. +' + rez.added.join(', ');
        return otvetMiniAppPosleDeystviya(tg_id, user, msg);
    }
    if (sub === 'remove') {
...
async function zapolnitIgruIzSpiskaVechera(igra, spisok) {
    ...
    const imm = await primenitVecherImmunitetNextKIgre(igra);
    if (imm) {
        igra._immunitet_next_msg = '🛡 Иммунитет с прошлой игры: №' + imm.nomer + ' ' + imm.name +
            ' (первая смерть: ночь 1 / 1-е голосование)';
    }
}
...
async function pokazatSostavSleduyushcheyIgryVechera(chatId, messageId, kod) {
    const igra = igry[kod];
    if (!igra) return;
    await zagruzitNazvanieKlubaVIgru(igra);
    igra.rezhim_rolei = 'karty';
    ...
    let t = '▶️ *Следующая игра вечера';
    if (igra.nomer_igry) t += ' №' + igra.nomer_igry;
    t += '*\n\n';
    if (nazvanieKlubaIgry(igra)) t += '\uD83C\uDFDB Клуб: *' + nazvanieKlubaIgry(igra) + '*\n';
    t += '\uD83C\uDFB4 Игра \u2116' + kod + '\n';
    t += '\uD83D\uDC65 Состав: *' + (igra.igroki || []).length + '*\n\n';
    (igra.igroki || []).forEach((p, i) => {
        t += (i + 1) + '. ' + (p.name || 'Игрок') +
            (estImmunitetOtGolosovaniya(p, igra) ? ' 🛡' : '') + '\n';
    });
    if (igra._immunitet_next_msg) {
        t += '\n' + igra._immunitet_next_msg + '\n';
        delete igra._immunitet_next_msg;
    }
    t += '\n_Проверь состав (кто ушёл / кто сел), затем начни ночь знакомства._\n';
    t += '_Роли раздаёте картами за столом — не через бота._';
    t += '\n_Иммунитет: промах ночи / первая смерть прошлой игры / вручную (🛡)._';
    ...
    if (igra.klub_id) {
        const pool = await poluchitSpisokVecheraKluba(igra.klub_id);
        const zaStolom = (igra.igroki || []).length;
        if (pool && pool.length > zaStolom) {
            knopki.push([{ text: '🌙 Пул вечера (' + pool.length + ') — кто приедет/уйдёт', callback_data: 'vecher_klub_' + igra.klub_id }]);
            knopki.push([{ text: '✍️ Заменить состав ЭТОЙ игры', callback_data: 'vecher_edit_' + kod }]);
        } else {
            knopki.push(...knopkiPravkiSostavaVechera(igra.klub_id));
            knopki.push([{ text: '✍️ Заменить весь состав', callback_data: 'vecher_edit_' + kod }]);
        }
        knopki.push([{ text: '🌙 Игровой вечер', callback_data: 'vecher_klub_' + igra.klub_id }]);
    } else {
        knopki.push([{ text: '\u270F\uFE0F Редактировать состав', callback_data: 'vecher_edit_' + kod }]);
    }
...
async function pokazatLobbyIgry(chatId, messageId, kod) {
    ...
    tekst += '\uD83C\uDFB4 Игра \u2116' + kod;
    if (igra.nomer_igry) tekst += ' · вечер №' + igra.nomer_igry;
    tekst += '\n';
...
async function pokazatMoiIgryBystraya(chatId, telegram_id) {
    ...
    aktivnye.forEach(({ kod, igra }) => {
        ...
        if (!igra.roli_razdany) estLobby = true;
    ...
    if (estLobby) knopki.push([{ text: '🧹 Очистить пустые лобби', callback_data: 'ochistit_lobby' }]);
...
async function pokazatIgrovoyVecher(chatId, messageId, klub, telegram_id) {
    ...
    if (spisok?.length && !zavershen) {
        const nextN = await predlozhennyyNomerIgryVechera(klubInfo.id, today);
        knopki.push([{ text: '▶️ Следующая игра (№' + nextN + ') · выбрать стол', callback_data: 'vecher_sozdat_igru_' + klubInfo.id }]);
    }
    ...
    if (!zavershen) {
        ...
        knopki.push(...knopkiPravkiSostavaVechera(klubInfo.id));
...
function tekstStatusaEskort(igra) {
    ...
    const skipRest = !!(igra?.noch_deystviya || {}).eskort_propustila;
    if (vybory.length === 0) {
        return skipRest ? 'пропустила' : ('не стреляла (' + limit + ' доступно)');
    }
    const chasti = vybory.map(v => '\u2116' + v.nomer + ' \u2192 ' + v.ugadannaya_rol);
    return chasti.join('; ') + ' (' + vybory.length + '/' + limit + ')' + (skipRest && vybory.length < limit ? ', остальные пропуск' : '');
}

function eskortNochZavershena(igra) {
    const d = igra?.noch_deystviya || {};
    if (d.eskort_propustila) return true;
    return eskortVyboryNochi(igra).length >= limitVystrelovEskort(igra);
}
...
async function otpravitVyborNachalaDnya(chatId, messageId, kod, itogPrefix) {
    ...
    await pokazatHostPanelVnizu(igra, chatId, messageId, t, knopki);
}
...
function opredelitPobeditelya(igra) {
    ...
    // Паскаль/спорт: мафия побеждает при равенстве или перевесе (4 мафии vs 4 мирных = конец).
    // Охотник/Стрелок/Маньяк не отменяют это условие — иначе игра «висит» при 4/4.
...
async function zavershitIgruAvto(chatId, messageId, kod, pobeditel) {
    ...
    const alive = (igra.igroki || []).filter(i => i.status === 'v_igre');
    const mafN = alive.filter(i => isMafiaRole(i.rol)).length;
    const manyakN = alive.filter(i => i.rol === 'Маньяк').length;
    const mirN = alive.length - mafN - manyakN;
    svodka += '_За столом: мафия ' + mafN + ' / мирные ' + mirN + (manyakN ? ' / маньяк ' + manyakN : '') + '_\n';
    if (alive.some(i => i.rol === 'Эскортница' || i.rol === 'Путана')) {
        svodka += '_Эскортница/Путана считается за мафию в подсчёте победы._\n';
    }
    svodka += '\nИгра *не завершена автоматически*, рейтинг не записан.\n';
    svodka += 'Подтверди итог вручную, если игра действительно закончилась.\n\n';
    svodka += '*Сейчас за столом:*\n';
    igra.igroki.forEach(i => {
        const em = i.status === 'v_igre' ? '✅' : '💀';
        svodka += em + ' №' + i.nomer + ' ' + md(i.name) + ' — ' + (i.rol || '?') + '\n';
    });

    const knopki = [
        [{ text: '🏁 Подтвердить итог игры', callback_data: 'konec_' + kod }],
        [{ text: '🎮 Продолжить игру', callback_data: 'prodolzhit_igru_' + kod }],
        [{ text: '\uD83C\uDF19 К ночи', callback_data: 'faza_noch_' + kod }],
        [{ text: '🏠 В меню', callback_data: 'menu_vedushchego' }]
    ];
    const payload = { parse_mode: 'Markdown', reply_markup: { inline_keyboard: knopki } };
    try {
        await bot.editMessageText(svodka, { chat_id: chatId, message_id: messageId, ...payload });
    } catch (e) {
        console.error('[zavershitIgruAvto]', kod, e?.message || e);
        await bot.sendMessage(chatId, svodka, payload).catch(() =>
            bot.sendMessage(chatId, svodka.replace(/[*_`]/g, ''), { reply_markup: payload.reply_markup })
        );
    }
    return true;
}
...
async function ustanovitPervogoHoda(chatId, messageId, kod, nomer, faza, telegram_id) {
    ...
    // Иммунитет первого/последнего круга больше не ставим — только первая смерть + ручной + промах ночи
    await sohranit_igru(kod);
...
async function zavershitKrugRechi(chatId, messageId, kod, opts = {}) {
    ...
    if (faza === 'den') {
        knopki.push([{ text: '\uD83D\uDCA5 К оправданию / списку', callback_data: 'vybrat_na_golos_' + kod }]);
...
async function sleduyushchiy(chatId, messageId, kod, opts = {}) {
    ...
    // Любой второй Пас/тик в течение 1.6 сек после перехода — это дубль, не следующий игрок.
    if (fromNomer != null && etoIstochnikAvtoHoda(source) && nedavnoPereshliNaTekushchego(igra, now)) {
        return dedupeOtvet();
    }
    ...
    try {
        ...
        if (fromLocked != null && etoIstochnikAvtoHoda(source) && nedavnoPereshliNaTekushchego(igra)) {
            return dedupeOtvet();
        }
...
function vystavitIgrokaNaGolos(igra, igrok) {
    ...
    // Иммунитет не мешает выставить на оправдание: игрок говорит второй раз,
    // а «не голосуют» — уже на этапе ввода голосов.
...
function knopkiRechiDen(igra, kod) {
    const knopki = [];
    // Выставление от текущего говорящего (1 за речь)
    if (igra?.tekushchiy_nomer) {
        if (uzheVystavilTekushchiyGovoryashchiy(igra)) {
            ...
            knopki.push([{ text: '\uD83D\uDCA5 Выставить (эта речь)', callback_data: 'vystav_nick_' + kod }]);
        }
    }
    // Пропущенного можно добавить в любой момент — таймер речи не останавливаем
    if (kandidatyDobavitNaGolos(igra).length > 0) {
        knopki.push([{ text: '\u2795 Добавить пропущенного', callback_data: 'golos_dobavit_' + kod }]);
    }
...
function knopkiPresetsTaymera(kod, faza) {
    if (faza === 'znakomstvo') {
        return [[{
            text: '⏱ Снова ' + ((igry[kod]?._nastroyki?.znakomstvo_sek) || 15) + 'с',
            callback_data: 'restart_taymer_' + kod
        }]];
    }
    if (faza === 'opravdanie') {
        const def = (igry[kod]?._nastroyki?.opravdanie_sek) || 30;
        return [[
            { text: '⏱ 20с', callback_data: 'set_opravdanie_' + kod + '_20' },
            { text: '⏱ ' + def + 'с', callback_data: 'set_opravdanie_' + kod + '_' + def },
            { text: '⏱ 40с', callback_data: 'set_opravdanie_' + kod + '_40' }
        ], [{ text: '▶️ Продолжить', callback_data: 'restart_taymer_' + kod }]];
    }
    // den
    return [[
...
function buildTimerKnopki(kod, faza) {
    ...
    if (faza === 'den') {
        ...
        if (igra_t?.tekushchiy_nomer) {
            ...
            knopki.push(...knopkiPresetsTaymera(kod, 'den'));
            ...
            // После круга речи — можно дописать пропущенных и идти к оправданию
            knopki.push(...knopkiRechiDen(igra_t, kod));
            knopki.push([{ text: '\uD83D\uDCA5 К оправданию / списку', callback_data: 'vybrat_na_golos_' + kod }]);
    ...
    if (faza === 'znakomstvo') {
        ...
        if (igra_z?.tekushchiy_nomer) {
            ...
            knopki.push(...knopkiPresetsTaymera(kod, 'znakomstvo'));
            knopki.push([{ text: '\u23ED\uFE0F Пропустить представление', callback_data: 'skip_krug_' + kod }]);
        }
    }
    if (faza === 'opravdanie') {
        if (igry[kod]?.tekushchiy_nomer) {
            knopki.push(...knopkiPresetsTaymera(kod, 'opravdanie'));
        }
        knopki.push([{ text: '\u270F\uFE0F Корректировать список', callback_data: 'vybrat_na_golos_' + kod }]);
        knopki.push([{ text: '\u26A0\uFE0F Выдать фол', callback_data: 'panel_foly_' + kod }]);
        knopki.push([{ text: '\uD83D\uDDF3 Голосование', callback_data: 'faza_golosovanie_' + kod }]);
...
function estImmunitetOtGolosovaniya(igrok, igra) {
    if (!igrok) return false;
    if (igrok.immunitet_snyat_vedushchim) return false;
    if (igrok.immunitet_ruchnoy) return true;
...
/** Ведущий вручную ставит/снимает иммунитет на эту игру */
function perekluchitImmunitetVedushchego(igra, nomer) {
    const igrok = igra?.igroki?.find(i => i.nomer === nomer && i.status === 'v_igre');
    if (!igrok) return null;
    if (estImmunitetOtGolosovaniya(igrok, igra)) {
        delete igrok.immunitet_posle_nochi;
        delete igrok.immunitet_ruchnoy;
        delete igrok.immunitet_golos;
        delete igrok.immunitet;
        delete igrok.bonus_immunitet;
        igrok.immunitet_snyat_vedushchim = true;
        return { igrok, on: false };
    }
    delete igrok.immunitet_snyat_vedushchim;
    igrok.immunitet_ruchnoy = true;
    return { igrok, on: true };
}
...
function primeniImmunitetyPosleNochi(igra, mishni) {
    ...
    for (const n of mishni || []) {
        ...
        if (igrok) {
            igrok.immunitet_posle_nochi = true;
            // Если ведущий раньше снимал щит — ночной промах всё равно даёт иммунитет на день
            delete igrok.immunitet_snyat_vedushchim;
        }
    }
}
...
function opisanieImmunitetaIgroka(igrok, igra) {
    const parts = [];
    if (igrok.immunitet_ruchnoy) parts.push('вручную');
    ...
    if (igrok.immunitet_posle_nochi) {
        parts.push(igra?.faza === 'znakomstvo' ? 'ожидает день' : 'после ночи / промах');
    }
    if (igrok._immunitet_iz_pervoy_smerti) parts.push('первая смерть прошлой игры');
...
async function pokazatPanelImmuniteta(chatId, messageId, kod) {
    const igra = igry[kod];
    if (!igra) return;
    // Как при «добавить пропущенного»: таймер речи не стопаем
    otkrytOverlayTaymera(igra, 'immunitet');
    await podgruzitImmunitetIgrokam(igra);
    await sohranit_igru(kod);

    let t = '\uD83D\uDEE1 *Иммунитет* — Игра \u2116' + kod + '\n\n';
    t += '_Нажми на игрока, чтобы поставить или снять щит._\n';
    t += '_Выставить на оправдание можно — голоса за щит не считают._\n\n';
    const sImm = (igra.igroki || []).filter(i => i.status === 'v_igre' && estImmunitetOtGolosovaniya(i, igra));
    if (!sImm.length) {
        t += '_Сейчас иммунитетов нет._\n';
    } else {
        t += '*Сейчас с \uD83D\uDEE1:*\n';
        sImm.forEach(i => {
    ...
    t += '\n_Авто: промах ночью → щит на день; первая смерть (ночь 1 / 1-е голосование) → щит на следующую игру вечера._';
    t += '\n_Ручной иммунитет можно ставить на любом этапе; действует до 2-го дня._';

    const knopki = [];
    (igra.igroki || []).filter(i => i.status === 'v_igre').forEach(i => {
        const on = estImmunitetOtGolosovaniya(i, igra);
        knopki.push([{
            text: (on ? '\uD83D\uDEE1 ' : '\u25FB\uFE0F ') + '\u2116' + i.nomer + ' ' + i.name,
            callback_data: 'imm_tog_' + kod + '_' + i.nomer
        }]);
    });
    ...
    const nazad = knopkaNazadKProcessu(igra, kod);
...
function tekstSpiskaNominacii(igra) {
    ...
    if (igra.taymer_aktiven && igra.tekushchiy_nomer && igra.taymer_sekundy > 0) {
        const cur = igra.igroki.find(i => i.nomer === igra.tekushchiy_nomer);
...
function knopkiSpiskaNominacii(igra, kod) {
    ...
    knopki.push([{ text: '\u2B05\uFE0F Назад', callback_data: knopkaNazadKProcessu(igra, kod) }]);
    return knopki;
}
...
function tekstDobavitNaGolos(igra) {
    ...
    t += '_Уже выставленные здесь не показываются. Иммунитет не мешает выставить: на оправдании скажет, голоса за него не вводят._';
    return t;
}
...
async function vypolnitVystavlenieNaGolos(chatId, igra, kod, igrok_vg, telegram_id, opts = {}) {
    ...
    if (!rez_vg.ok) {
        if (!silent && chatId) {
            const msg_vg = rez_vg.error === 'vybyl'
                ? '\u26A0\uFE0F Игрок уже выбыл.'
    ...
    delete igra._picker_type;
    await sohranit_igru(kod);
    // Таймер речи не трогаем: если шёл — продолжает, если нет — поднимаем
    if (!silent && chatId) {
        await vernutPanelTaymera(igra, kod, chatId, opts.messageId, {
            restartTimer: !igra.taymer_aktiven,
            suffix: '\n\n\uD83D\uDCA5 Выставлен \u2116' + igrok_vg.nomer + ' *' + igrok_vg.name + '*'
        });
    } else {
        obnovitPanelTaymera(kod);
        if (igra.tekushchiy_nomer && !igra._krug_zavershen && !igra.taymer_aktiven) {
            perezapustitTaymerRechi(igra, kod);
        }
    }
    return true;
}
...
/** Иммунитет: на оправдание выставляют, голоса за него не считают */
function oznachitImmunitetVGolosovanii(igra) {
    if (!igra) return;
    igra.golosa_dnya = igra.golosa_dnya || {};
    for (const i of nominirovannyePoPoryadku(igra)) {
        if (estImmunitetOtGolosovaniya(i, igra)) igra.golosa_dnya[i.nomer] = 0;
    }
}

function naznachenyBezImmuniteta(igra) {
...
function tekstGolosovaniyaSPodschetom(igra, kod) {
    ...
    if (naznacheny.length === 0) {
        ...
        naznacheny.forEach((i, idx) => {
            if (estImmunitetOtGolosovaniya(i, igra)) {
                t += (idx + 1) + '. \u2116' + i.nomer + ' ' + i.name + ' — \uD83D\uDEE1 *иммунитет, не голосуем*\n';
                return;
            }
...
function knopkiGolosovaniyaSPodschetom(igra, kod) {
    ...
    knopki.push([{ text: '\u270F\uFE0F Список выставленных', callback_data: 'vybrat_na_golos_' + kod }]);
    knopki.push([knopkaImmuniteta(kod)]);
...
function shagiNochiDeystviy(igra) {
    ...
    if (igraEstZhivayaRol(igra, 'Эскортница')) {
        const limit = limitVystrelovEskort(igra);
        const used = eskortVyboryNochi(igra).length;
        const skipRest = !!(igra.noch_deystviya || {}).eskort_propustila;
        const n = skipRest ? used : limit;
        for (let v = 1; v <= n; v++) {
            steps.push({
                rol: 'Эскортница',
                tip: 'eskort',
...
function tekstVyboraNochiGuided(igra, kod, step, idx, vsego) {
    ...
    if (nazvanieKlubaIgry(igra)) t += '\uD83C\uDFDB Клуб: *' + nazvanieKlubaIgry(igra) + '*\n';
    if (igra._noch_last_ok) t += '\u2705 *Было:* ' + igra._noch_last_ok + '\n\n';
    t += 'Сейчас: *' + step.label + '*\n';
    t += '_Шаг ' + (idx + 1) + '/' + vsego + '_\n\n';
    t += 'Нажми ник — выбор сразу засчитается.\n';
    t += '_' + tekstPodskazkiPoiskaIgroka().replace(/\n/g, ' ') + '_';
    ...
    if (step.tip === 'shahid') {
        ...
        t += '\n_Отметь ники галочками, затем «Готово»._';
    }
    if (step.tip === 'eskort') {
        t += '\n_После ника назови роль — угадала = убийство._';
        t += '\n_«Пропустить» = остальные выстрелы эскортницы тоже пропуск._';
    }
    const cur = tekushchiyVyborNochi(igra, step.tip, step);
    if (cur != null) {
        const igrokCur = typeof cur === 'number' ? igra.igroki.find(i => i.nomer === cur) : null;
        const curTxt = cur === 'пропуск'
            ? 'пропуск'
            : (igrokCur ? '\u2116' + cur + ' *' + igrokCur.name + '*' : String(cur));
        t += '\n\n\u2705 *Выбрано: ' + curTxt + '*';
    }
    return t;
}
...
function knopkiShagaNochiGuided(igra, kod, step, idx) {
    ...
    if (step.tip === 'shahid') {
        ...
        const knopki = ryadyKnopokPoN(seatBtns, 2);
        knopki.push([{ text: '\u2705 Готово (' + vybrany.length + '/' + limit + ')', callback_data: 'noch_g_next_' + kod }]);
    ...
    if (step.tip === 'strelok' || step.tip === 'eskort' || step.tip === 'kami') {
        const skipLabel = step.tip === 'eskort'
            ? '\u23ED Пропустить (и остальные выстрелы)'
            : '\u23ED Пропустить выстрел';
        knopki.unshift([{ text: skipLabel, callback_data: 'noch_g_skip_' + kod + '_' + idx }]);
    }
    if (cur != null) {
        knopki.push([{ text: '\u274C Сбросить выбор', callback_data: 'noch_g_sbr_' + kod + '_' + idx }]);
        knopki.push([{ text: '\u2705 Далее — выбор верный', callback_data: 'noch_g_next_' + kod }]);
    }
    if (idx > 0) knopki.push([{ text: '\u2B05 Предыдущая роль', callback_data: 'noch_g_prev_' + kod }]);
    knopki.push([{ text: '\uD83D\uDCCB Классическая панель', callback_data: 'noch_panel_' + kod }]);
    return knopki;
...
function idxPervogoNezavershennogoShaga(igra, startIdx = 0) {
    const shagi = shagiNochiDeystviy(igra);
    for (let i = Math.max(0, startIdx); i < shagi.length; i++) {
        if (tekushchiyVyborNochi(igra, shagi[i].tip, shagi[i]) == null) return i;
    }
    return shagi.length;
}

async function pokazatHostPanelVnizu(igra, chatId, messageId, text, knopki) {
    const markup = { inline_keyboard: knopki || [] };
    let sent = null;
    try {
        sent = await bot.sendMessage(chatId, text, { parse_mode: 'Markdown', reply_markup: markup });
    } catch (_) {
        sent = await bot.sendMessage(chatId, String(text || '').replace(/[*_`]/g, ''), { reply_markup: markup }).catch(() => null);
    }
    if (sent?.message_id && igra) {
        igra._live_chat_id = chatId;
        igra._live_message_id = sent.message_id;
        igra._taymer_chat_id = chatId;
        igra._taymer_message_id = sent.message_id;
    }
    if (chatId && messageId && sent?.message_id && messageId !== sent.message_id) {
        bot.editMessageReplyMarkup({ inline_keyboard: [] }, { chat_id: chatId, message_id: messageId }).catch(() => {});
    }
    return sent;
}

async function pokazatHostPanel(igra, chatId, messageId, text, knopki, opts = {}) {
    if (!chatId) return null;
    if (opts.vnizu || !messageId) return pokazatHostPanelVnizu(igra, chatId, messageId, text, knopki);
    const markup = { inline_keyboard: knopki || [] };
    try {
        await bot.editMessageText(text, {
            chat_id: chatId, message_id: messageId, parse_mode: 'Markdown', reply_markup: markup
        });
        if (igra) {
            igra._live_chat_id = chatId;
            igra._live_message_id = messageId;
        }
        ...
        try {
            ...
            return pokazatHostPanelVnizu(igra, chatId, messageId, text, knopki);
        }
    }
}

function klonIgryDlyaItogovNochi(igra) {
    return {
        den: igra.den,
        kolichestvo: igra.kolichestvo,
        igroki: (igra.igroki || []).map(i => ({
            nomer: i.nomer,
            name: i.name,
            rol: i.rol,
            status: i.status,
            mirnye_ubitye_strelkom: i.mirnye_ubitye_strelkom
        })),
        noch_deystviya: JSON.parse(JSON.stringify(igra.noch_deystviya || {})),
        shahid_miny: [...(igra.shahid_miny || [])],
        _nastroyki: igra._nastroyki,
        doctor_proshlaya_tsel: igra.doctor_proshlaya_tsel,
        doctor_sebya_podryad: igra.doctor_sebya_podryad,
        avto_bonusy: []
    };
}

function tekstChernovikaItogovNochi(igra) {
    const clone = klonIgryDlyaItogovNochi(igra);
    const d = clone.noch_deystviya || {};
    let t = '\uD83D\uDCCB *Черновик итогов ночи ' + (clone.den || 1) + '*\n\n_Ещё не применено — можно исправить._\n\n';
    if (d.don_tseli) {
        const proverka_don = clone.igroki.find(x => x.nomer === d.don_tseli);
        t += '\uD83D\uDD0E Дон \u2192 \u2116' + d.don_tseli + ': ' + (proverka_don && isSheriffRole(proverka_don.rol) ? 'Шериф/Комиссар' : 'не Шериф') + '\n';
    }
    if (d.sherif_tseli) {
        const proverka_sher = clone.igroki.find(x => x.nomer === d.sherif_tseli);
        t += '\uD83D\uDD0D Шериф \u2192 \u2116' + d.sherif_tseli + ': ' + (proverka_sher && isMafiaRole(proverka_sher.rol) ? 'Мафия' : 'мирный') + '\n';
    }
    if (d.kons_tseli) t += '\uD83E\uDD1D Консильери \u2192 \u2116' + d.kons_tseli + '\n';
    if (d.shahid_miny_tseli && d.shahid_miny_tseli.length) {
        t += '\uD83D\uDCA3 Шахид: ' + d.shahid_miny_tseli.map(n => '\u2116' + n).join(', ') + '\n';
    }
    if (d.shahid_miny_tseli) clone.shahid_miny = [...d.shahid_miny_tseli];
    const vystrely = primenitNochnyeVystrely(clone, d);
    vystrely.lines.forEach(line => { t += line; });
    (d.eskort_vybory || []).forEach(v => {
        const tselEsk = clone.igroki.find(x => x.nomer === v.nomer);
        if (!tselEsk || tselEsk.status !== 'v_igre') {
            t += '\uD83D\uDC8B Эскортница: \u2116' + v.nomer + ' \u2192 ' + v.ugadannaya_rol + ' (уже не за столом)\n';
            return;
        }
        if (sravnitRoliEskort(v.ugadannaya_rol, tselEsk.rol)) {
            tselEsk.status = 'vybyl';
            t += '\uD83D\uDC8B Эскортница угадала: \u2116' + tselEsk.nomer + ' *' + tselEsk.name + '* — ' + tselEsk.rol + '\n';
        } else {
            t += '\uD83D\uDC8B Эскортница промах: \u2116' + tselEsk.nomer + ' *' + tselEsk.name + '* не ' + v.ugadannaya_rol + '\n';
        }
    });
    const v_igre_t = clone.igroki.filter(i => i.status === 'v_igre');
    t += '\n\uD83D\uDC65 *Останутся за столом: ' + v_igre_t.length + '*\n';
    v_igre_t.forEach(i => { t += '\u2705 \u2116' + i.nomer + ' ' + md(i.name) + '\n'; });
    t += '\nНажми *Применить*, если всё верно, или *Исправить* — если не ту кнопку.';
    return t;
}
...
async function pokazatShagNochiGuided(chatId, messageId, kod) {
    ...
    await pokazatHostPanel(igra, chatId, messageId, t, knopki);
}
...
async function pokazatSvodkuNochiGuided(chatId, messageId, kod) {
    ...
    if (igra._noch_last_ok) t += '\u2705 *Последний выбор:* ' + igra._noch_last_ok + '\n\n';
    shagi.forEach((step) => {
        const cur = tekushchiyVyborNochi(igra, step.tip, step);
        if (cur == null) return;
        t += '\u2705 ' + step.label;
    ...
    const nezavershennye = shagi
        .map((step, idx) => ({ step, idx, cur: tekushchiyVyborNochi(igra, step.tip, step) }))
        .filter(x => x.cur == null);
    if (nezavershennye.length) {
        t += '\n_Не отмечено: ' + nezavershennye.map(x => x.step.label).join(', ') + '._\n';
    }
    if ((igra.noch_deystviya || {}).eskort_propustila) {
        t += '\n_Остальные выстрелы эскортницы — пропуск._';
    }
    ...
    const knopki = [];
    nezavershennye.forEach(({ step, idx }) => {
        knopki.push([{ text: step.label, callback_data: 'noch_g_redo_' + kod + '_' + idx }]);
    });
    knopki.push([{ text: '\u270F\uFE0F Исправить действия', callback_data: 'noch_guided_' + kod }]);
...
async function primeniIntroMirnyhVseh(igra, kod) {
    const bezRoli = igrokiBezRoliDlyaMirnyh(igra);
    const nuzhno = mirnyeOstalosVnesti(igra);
    if (bezRoli.length === 0 || nuzhno <= 0) {
        const fin = await zavershitNochZnakomstva(null, kod, { silent: true });
        if (fin?.ok === false) {
            return { ok: false, message: fin.message || fin.paywall || 'Не удалось завершить ночь знакомства', done: false };
        }
    ...
    return {
        ok: true,
        message: 'Мирные подтверждены (' + bezRoli.length + '). Ночь знакомства завершена.',
        done: true
    };
}
...
async function miniAppHostAction(tg_id, user, body) {
    ...
    const HOST_FAZY = {
        ...
        start_znakomstvo: ['lobby', 'registraciya', 'znakomstvo', 'ozhidanie', ''],
        start_day: ['lobby', 'registraciya', 'znakomstvo', 'ozhidanie', ''],
        ...
        intro_clear: ['lobby', 'registraciya', 'znakomstvo', 'ozhidanie', 'noch_znakomstvo', ''],
    ...
    if (sub === 'skip_krug') {
        ...
        await zavershitKrugRechi(igra._taymer_chat_id || null, igra._taymer_message_id || null, kod, { skipped: true });
    ...
    if (sub === 'night_pick') {
        ...
        if (!rez.toggle) igra._noch_guided_idx = idxPervogoNezavershennogoShaga(igra, idx + 1);
        await sohranit_igru(kod);
    ...
    if (sub === 'night_skip') {
        ...
        if (step?.tip === 'eskort') {
            igra.noch_deystviya = igra.noch_deystviya || {};
            igra.noch_deystviya.eskort_propustila = true;
            igra._noch_guided_idx = idxPervogoNezavershennogoShaga(igra, 0);
            await sohranit_igru(kod);
            return otvetMiniAppPosleDeystviya(tg_id, user, 'Эскортница: остальные выстрелы — пропуск');
        }
    ...
    if (sub === 'intro_mirny') {
        if (body.confirm_all) {
            const rez = await primeniIntroMirnyhVseh(igra, kod);
            return otvetMiniAppPosleDeystviya(tg_id, user, rez.message);
        }
        if (body.text) {
    ...
    if (sub === 'intro_clear') {
        ...
        const igrok = igra.igroki.find(i => i.nomer === nomer);
        if (!igrok?.rol || igrok.rol === 'Мирный') {
            return { stay: true, message: 'У игрока нет активной роли' };
        }
        const rol = igrok.rol;
        delete igrok.rol;
        const idx = indeksShagaDlyaRoli(igra, rol);
        sinhronizirovatMiniappIntro(igra, idx);
        ustanovitSostoyanieNochiZnakomstva(igra, igra.vedushchii_id, kod, idx);
        await sohranit_igru(kod);
        return otvetMiniAppPosleDeystviya(tg_id, user, 'Снята роль ' + rol + ' с №' + nomer + '. Введи заново.');
    }
    if (sub === 'pick_first') {
        const nomer = parseInt(body.nomer, 10);
    ...
    if (sub === 'immunity') {
        ...
        const msg = immune.length
            ? 'Иммунитет: ' + immune.map(i => '№' + i.nomer + ' ' + i.name).join(', ') + '. В боте: «🛡 Иммунитет» — нажми игрока, чтобы снять/поставить.'
            : 'Активных иммунитетов нет. В боте: «🛡 Иммунитет» — можно поставить вручную.';
        return otvetMiniAppPosleDeystviya(tg_id, user, msg);
    }
    if (sub === 'immunity_toggle') {
        const nomer = parseInt(body.nomer, 10);
        const rez = perekluchitImmunitetVedushchego(igra, nomer);
        if (!rez) return { stay: true, message: 'Игрок не найден' };
        await sohranit_igru(kod);
        obnovitPanelTaymera(kod);
        return otvetMiniAppPosleDeystviya(tg_id, user, rez.on
            ? 'Иммунитет поставлен: №' + nomer + ' ' + rez.igrok.name
            : 'Иммунитет снят: №' + nomer + ' ' + rez.igrok.name);
    }
    if (sub === 'start_znakomstvo') {
        ...
        // В миниаппе нет chatId для кнопок бота — оставляем режим выбора первого
        igra._pick_first_faza = 'znakomstvo';
        igra.faza = 'ozhidanie';
        await sohranit_igru(kod);
        return otvetMiniAppPosleDeystviya(tg_id, user, 'Выбери, кто начинает представление (клик по месту или «случайно»)');
    }
    if (sub === 'start_day') {
        await podgruzitNastroykiIgry(igra);
        const n = igra._nastroyki || {};
        ...
        igra.tekushchiy_nomer = null;
        if (n.perviy_hod_avto && n.perviy_hod_nomer) {
            await ustanovitPervogoHoda(null, null, kod, n.perviy_hod_nomer, 'den', tg_id);
            return otvetMiniAppPosleDeystviya(tg_id, user, 'День начат');
        }
        await sohranit_igru(kod);
        return otvetMiniAppPosleDeystviya(tg_id, user, 'Выбери, кто начинает день (клик по месту или «случайно»)');
    }
    if (sub === 'start_voting') {
    ...
    if (sub === 'vote_set') {
        ...
        const igrokVote = igra.igroki.find(i => i.nomer === nomer);
        if (estImmunitetOtGolosovaniya(igrokVote, igra)) {
            return { stay: true, message: 'У игрока иммунитет — голоса за него не вводят' };
        }
        igra.golosa_dnya = igra.golosa_dnya || {};
...
async function pokazat_noch_panel(chatId, messageId, kod, log_msg) {
    ...
    if (roli_alive.includes('Доктор')) {
        t += (d.doctor_tseli ? '\u2705' : '\u25A1') + ' Доктор: ' + (d.doctor_tseli ? '\u2116' + d.doctor_tseli : 'не выбрал') + '\n';
    } else {
        const docDead = (igra.igroki || []).find(i => i.rol === 'Доктор');
        if (docDead) {
            t += '\uD83D\uDC80 Доктор: выбыл (\u2116' + docDead.nomer + ' ' + md(docDead.name || '') + ') — этой ночью не лечит\n';
        }
    }
    if (roli_alive.some(rolStrelyayushchegoZaMirnyh)) {
        ...
        const strDead = (igra.igroki || []).find(i => rolStrelyayushchegoZaMirnyh(i.rol));
        if (strDead && strDead.status !== 'v_igre') {
    ...
    const knopki = [];
    if (!(d.mafiya_tseli || []).length) knopki.push([{ text: '\uD83D\uDD2B Мафия убивает', callback_data: 'noch_vybor_maf_' + kod }]);
    if (roli_alive.includes('Дон') && !d.don_tseli) knopki.push([{ text: '\uD83D\uDD0E Дон ищет Шерифа', callback_data: 'noch_vybor_don_' + kod }]);
    if (roli_alive.includes('Эскортница') && !eskortNochZavershena(igra)) {
    ...
    if (roli_alive.includes('Консильери') && mozhetKonsilyeriVerbovat(igra) && !d.kons_tseli) knopki.push([{ text: '\uD83E\uDD1D Консильери вербует', callback_data: 'noch_vybor_kons_' + kod }]);
    if (roli_alive.some(isSheriffRole) && !d.sherif_tseli) knopki.push([{ text: '\uD83D\uDD0D Шериф/Комиссар проверяет', callback_data: 'noch_vybor_sher_' + kod }]);
    if (roli_alive.includes('Маньяк') && !d.manyak_tseli) knopki.push([{ text: '\uD83C\uDFAF Маньяк стреляет', callback_data: 'noch_vybor_manyak_' + kod }]);
    if (roli_alive.includes('Доктор') && !d.doctor_tseli) knopki.push([{ text: '\uD83D\uDC89 Доктор лечит', callback_data: 'noch_vybor_doc_' + kod }]);
    if (roli_alive.some(rolStrelyayushchegoZaMirnyh) && !strelokNochZavershen(d)) {
    ...
    if (roli_alive.includes('Камикадзе') && !d.kamikadze_tseli && !d.kamikadze_propustil) {
    ...
    if (roli_alive.includes('Затычка') && !d.zatychka_tseli) knopki.push([{ text: '\uD83D\uDD07 Затычка блокирует', callback_data: 'noch_vybor_zat_' + kod }]);
    if (roli_alive.includes('Шахид') && (igra.den === 1 || igra.den === 2)) {
...
function sozdatPreviewSostava(opts) {
    ...
    igry['preview_' + id] = {
        ...
        vecher_igroki: Array.isArray(opts.vecher_igroki) ? opts.vecher_igroki : null,
...
bot.on('callback_query', async function(query) {
    ...
    if (!etoCallbackBezRegistracii(data) && await zablokirovatNezaregistrirovannogo(chatId, telegram_id)) {
        bot.answerCallbackQuery(query.id, { text: 'Сначала регистрация по приглашению', show_alert: true }).catch(() => {});
        return;
    }
    ...
    if (data === 'menu_vedushchego' || data === 'menu_vladeltsa' || data === 'menu_igroka') {
        ...
        bot.editMessageText(
            '➕ *Добавить игрока*\n\n' +
            'Кто *сел дополнительно* — пришли ник (или несколько).\n\n' +
            'Пример: `Аня` или `Аня, Оля`\n\n' +
            '_Если кто-то *уехал*, а на его место сел другой — кнопка «🔄 Заменить игрока»._',
            {
                chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [
                    [{ text: '🔄 Заменить игрока', callback_data: 'vecher_replace_' + klub_id }],
    ...
    else if (data.startsWith('vecher_replace_')) {
        const klub_id = data.replace('vecher_replace_', '');
        bot.answerCallbackQuery(query.id).catch(() => {});
        await pokazatVyborMestaVechera(chatId, messageId, klub_id, 'rep');
    }

    else if (data.startsWith('vecher_delete_')) {
        const klub_id = data.replace('vecher_delete_', '');
        bot.answerCallbackQuery(query.id).catch(() => {});
        await pokazatVyborMestaVechera(chatId, messageId, klub_id, 'del');
    }

    else if (data.startsWith('vecher_delpick_')) {
        const parsed = parseVecherSeatCb(data, 'vecher_delpick_');
        if (!parsed) return;
        const { klub_id, nomer } = parsed;
        const rez = await ubratIgrokovIzSpiskaVechera(klub_id, String(nomer));
        if (!rez.ok) {
            bot.answerCallbackQuery(query.id, { text: 'Не удалось удалить', show_alert: true }).catch(() => {});
            return;
        }
        bot.answerCallbackQuery(query.id, { text: 'Удалён №' + nomer }).catch(() => {});
        await posleIzmeneniyaSpiskaVechera(chatId, telegram_id, klub_id,
            '✅ Удалил место №' + nomer + '. Осталось: *' + rez.count + '*.');
    }

    else if (data.startsWith('vecher_reppick_')) {
        const parsed = parseVecherSeatCb(data, 'vecher_reppick_');
        if (!parsed) return;
        const { klub_id, nomer } = parsed;
        const spisok = await poluchitSpisokVecheraKluba(klub_id);
        const cur = spisok?.[nomer - 1];
        sostoyanie[telegram_id] = 'vecher_repnick_' + klub_id + '_' + nomer;
        bot.answerCallbackQuery(query.id, { text: 'Место №' + nomer }).catch(() => {});
        bot.editMessageText(
            '🔄 *Замена места №' + nomer + '*\n\n' +
            'Сейчас: *' + md(cur?.name || '—') + '*\n\n' +
            'Пришли *новый ник* того, кто сел вместо него.',
            {
                chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [
                    [{ text: '⬅️ Выбрать другое место', callback_data: 'vecher_replace_' + klub_id }],
                    [{ text: '🗑 Просто удалить это место', callback_data: 'vecher_delpick_' + klub_id + '_' + nomer }],
                    [{ text: '⬅️ Назад', callback_data: 'vecher_klub_' + klub_id }]
                ] }
            }
        );
        ...
        // Всегда выбор «кто за столом» — при 20 в пуле не все играют игру 1
        delete vecherStolPick[telegram_id];
        await pokazatVyborStolaIgry(chatId, messageId, klub_id, telegram_id);
    }

    else if (data.startsWith('vecher_stol_tog_')) {
        const rest = data.replace('vecher_stol_tog_', '');
        const idxSep = rest.lastIndexOf('_');
        if (idxSep < 0) return;
        const klub_id = rest.slice(0, idxSep);
        const idx = parseInt(rest.slice(idxSep + 1), 10);
        const spisok = await poluchitSpisokVecheraKluba(klub_id);
        if (!spisok?.length || !Number.isFinite(idx) || idx < 0 || idx >= spisok.length) return;
        const st = poluchitIliSozdatVyborStola(telegram_id, klub_id, spisok.length);
        if (st.selected.has(idx)) st.selected.delete(idx);
        else st.selected.add(idx);
        await pokazatVyborStolaIgry(chatId, messageId, klub_id, telegram_id);
    }

    else if (data.startsWith('vecher_stol_all_')) {
        const klub_id = data.replace('vecher_stol_all_', '');
        const spisok = await poluchitSpisokVecheraKluba(klub_id);
        if (!spisok?.length) return;
        vecherStolPick[telegram_id] = {
            klub_id,
            selected: new Set(Array.from({ length: spisok.length }, (_, i) => i))
        };
        await pokazatVyborStolaIgry(chatId, messageId, klub_id, telegram_id);
    }

    else if (data.startsWith('vecher_stol_clr_')) {
        const klub_id = data.replace('vecher_stol_clr_', '');
        const spisok = await poluchitSpisokVecheraKluba(klub_id);
        if (!spisok?.length) return;
        vecherStolPick[telegram_id] = { klub_id, selected: new Set() };
        await pokazatVyborStolaIgry(chatId, messageId, klub_id, telegram_id);
    }

    else if (data.startsWith('vecher_stol_ok_')) {
        const klub_id = data.replace('vecher_stol_ok_', '');
        await pereytiKPreviewIzVyborStola(chatId, messageId, klub_id, telegram_id);
    }

    else if (data.startsWith('vecher_stol_')) {
        // Назад с превью ролей к выбору стола
        const klub_id = data.replace('vecher_stol_', '');
        await pokazatVyborStolaIgry(chatId, messageId, klub_id, telegram_id);
    }
    ...
    // ===== МОИ ИГРЫ ВЕДУЩЕЙ =====
    ...
    else if (data === 'ochistit_lobby') {
        const n = await ochistitLobbyVedushchegoBezRoley(telegram_id);
        bot.answerCallbackQuery(query.id, {
            text: n ? ('Удалено лобби: ' + n) : 'Пустых лобби нет'
        });
        const aktivnye = aktivnyeIgryVedushchego(telegram_id);
        if (aktivnye.length === 0) {
            bot.editMessageText(
                '🎮 *Мои игры*\n\nАктивных игр пока нет.' + (n ? '\n\n_Очищено лобби: ' + n + '_' : ''),
                { chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
                  reply_markup: { inline_keyboard: [
                    [{ text: '🎲 Создать игру', callback_data: 'sozdat_igru' }],
                    [{ text: '📚 История игр', callback_data: 'istoriya_igr' }],
        ...
        let t = '🎮 *Мои активные игры*\n\n';
        if (n) t += '_Очищено пустых лобби: ' + n + '_\n\n';
        let estLobby = false;
        const knopki = aktivnye.map(({ kod, igra }) => {
            const vIgre = (igra.igroki || []).filter(i => i.status === 'v_igre').length;
            const rezhim = igra.rezhim_rolei === 'karty' ? 'физ. карты' : (igra.rezhim_rolei === 'bot' ? 'бот' : 'режим не выбран');
    ...
    // ===== ВЕДУЩИЙ: создать игру =====
        ...
        if (kluby.length === 1) {
            const klubOd = kluby[0];
            const dannyeVecher = await poluchitDannyeVecheraKluba(klubOd.id);
            if (dannyeVecher.spisok?.length && !dannyeVecher.zavershen) {
                const nextN = await predlozhennyyNomerIgryVechera(klubOd.id, dataIgrovoegoVechera());
                bot.editMessageText(
                    '🎲 *Создание игры*\n\nКлуб: *' + md(klubOd.nazvaniye || '') + '*\n\n' +
                    'Идёт игровой вечер — состав уже есть (*' + dannyeVecher.spisok.length + '* чел.).\n' +
                    'Следующая партия: *№' + nextN + '*.',
                    {
                        chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
                        reply_markup: { inline_keyboard: [
                            [{ text: '▶️ Следующая игра вечера (№' + nextN + ')', callback_data: 'vecher_sozdat_igru_' + klubOd.id }],
                            [{ text: '🌙 Игровой вечер', callback_data: 'vecher_klub_' + klubOd.id }],
                            [{ text: '🎲 Создать с нуля', callback_data: 'igra_bez_anons_' + klubOd.id }],
                            [nazad]
                        ]}
                    }
                );
            ...
            bot.editMessageText('🎲 *Создание игры*\n\nКлуб: *' + klubOd.nazvaniye + '*\n\nЭта игра по анонсу?', {
                chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [
                    [{ text: '📢 По анонсу', callback_data: 'igra_anons_' + klubOd.id }],
                    [{ text: '🎲 Без анонса', callback_data: 'igra_bez_anons_' + klubOd.id }],
                    [nazad]
                ]}
            });
            return;
        ...
        const { data: klub } = await supabase.from('kluby').select('nazvaniye').eq('id', klub_id).single();
        const dannyeVecher = await poluchitDannyeVecheraKluba(klub_id);
        if (dannyeVecher.spisok?.length && !dannyeVecher.zavershen) {
            const nextN = await predlozhennyyNomerIgryVechera(klub_id, dataIgrovoegoVechera());
            bot.editMessageText(
                '🎲 *Создание игры*\n\nКлуб: *' + md(klub?.nazvaniye || '') + '*\n\n' +
                ...
                'Следующая партия: *№' + nextN + '*.',
                {
                    chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
                    reply_markup: { inline_keyboard: [
                        [{ text: '▶️ Следующая игра вечера (№' + nextN + ')', callback_data: 'vecher_sozdat_igru_' + klub_id }],
                        [{ text: '🌙 Игровой вечер', callback_data: 'vecher_klub_' + klub_id }],
                        [{ text: '🎲 Создать с нуля', callback_data: 'igra_bez_anons_' + klub_id }],
    ...
    // ===== СОСТАВ: подтвердить и создать игру =====
        ...
        if (preview_data.igra_kod && igry[preview_data.igra_kod]) {
            ...
            await sohranit_igru(kod);
            if (preview_data.klub_id || igry[kod].klub_id) {
                await sohranitKastomnyySostavKluba(
                    preview_data.klub_id || igry[kod].klub_id,
                    preview_data.kolichestvo || igry[kod].kolichestvo,
                    preview_data.sostav
                );
            }
        ...
        // Следующая игра вечера — создать с выбранным составом ролей и столом
        if (preview_data.from_vecher && preview_data.klub_id) {
            const kod = await sozdatIgryIzVechera(telegram_id, preview_data.klub_id, {
                igroki: preview_data.vecher_igroki || null
            });
            if (!kod) {
                await soobshitPosleKnopki(chatId, '❌ Сначала зафиксируй состав вечера / выбери кто за столом.');
                return;
            }
            igry[kod]._sostav_custom = [...preview_data.sostav];
            await sohranitKastomnyySostavKluba(preview_data.klub_id, preview_data.kolichestvo, preview_data.sostav);
            await sohranit_igru(kod);
            delete igry['preview_' + preview_key];
            delete vecherStolPick[telegram_id];
            await soobshitPosleKnopki(chatId,
                '✅ Игра вечера №' + (igry[kod]?.nomer_igry || '?') +
                ' создана · за столом ' + (igry[kod]?.igroki?.length || preview_data.kolichestvo));
            await pokazatSostavSleduyushcheyIgryVechera(chatId, messageId, kod);
            return;
        }
    ...
    // ===== СОСТАВ: сбросить к стандарту =====
    ...
    else if (data.startsWith('nz_seat_')) {
        const parts = data.replace('nz_seat_', '').split('_');
        const nomer = parseInt(parts[parts.length - 1], 10);
        const kod = parts.slice(0, -1).join('_');
        const igra = igry[kod];
        if (!igra || !Number.isFinite(nomer)) return;
        const st = sostoyanie[telegram_id] || sostoyanie[igra.vedushchii_id] || '';
        const parsed = razobratSostoyanieNochiZnakomstva(st);
        const idx = parsed?.kod === kod ? (parsed.idx || 0) : 0;
        const roles = poryadokRoleyDlyaNochi(igra);
        const rol = roles[idx];
        if (!rol) {
        ...
        const igrok = igra.igroki.find(i => i.nomer === nomer);
        if (!igrok) {
            await bot.sendMessage(chatId, '❌ Место №' + nomer + ' не найдено в составе.').catch(() => {});
            return;
        }
        if (igrok.rol) {
            await bot.sendMessage(chatId,
                '⚠️ У №' + nomer + ' *' + md(igrok.name) + '* уже роль *' + igrok.rol + '*.',
                { parse_mode: 'Markdown' }
            ).catch(() => {});
            return;
        }
        ...
        igrok.foly = igrok.foly || 0;
        await sohranit_igru(kod);
        await bot.sendMessage(chatId, '✅ №' + igrok.nomer + ' ' + igrok.name + ' — *' + rol + '*', { parse_mode: 'Markdown' }).catch(() => {});
        await pokazatShagNochiZnakomstva(chatId, kod, idx + 1, telegram_id);
    }
    ...
    else if (data.startsWith('nz_restart_')) {
    ...
    // ===== ПАНЕЛЬ ИММУНИТЕТА =====
    ...
    else if (data.startsWith('imm_tog_')) {
        const parts = data.replace('imm_tog_', '').split('_');
        const kod = parts[0];
        const nomer = parseInt(parts[1], 10);
        const igra = igry[kod];
        if (!igra || !Number.isFinite(nomer)) return;
        ...
        bot.answerCallbackQuery(query.id, {
            text: rez.on
                ? '\uD83D\uDEE1 Иммунитет: \u2116' + nomer + ' ' + rez.igrok.name
                : 'Снят иммунитет: \u2116' + nomer + ' ' + rez.igrok.name
        });
        await pokazatPanelImmuniteta(chatId, messageId, kod);
    }

    // ===== ИГРОВАЯ ПАНЕЛЬ =====
    else if (data.startsWith('panel_') && !data.startsWith('panel_foly_') && !data.startsWith('panel_mirny_') && !data.startsWith('panel_immunitet_') && !data.startsWith('panel_vybyl_')) {
        ...
        if (igra.faza) tekst += 'Фаза: *' + (igra.faza || '') + '*\n';
        tekst += '\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n';

        igra.igroki.filter(i => i.status === 'v_igre').forEach(igrok => {
            let emoji = igrok.foly > 0 ? '\u26A0\uFE0F' : '\u2705';
            tekst += emoji + ' \u2116' + igrok.nomer + ' *' + igrok.name + '* — ' + (igrok.rol || '?');
            if (igrok.foly > 0) tekst += ' [' + igrok.foly + ' фол]';
            if (estImmunitetOtGolosovaniya(igrok, igra)) tekst += ' \uD83D\uDEE1';
            tekst += '\n';
        });
        if (dead.length) {
        ...
        // Сначала кнопки процесса — не «выбыл» на каждого (из‑за этого зависали после голосования)
        const knopki = [];
        if (igra._posle_golosa) {
            knopki.push([{ text: '\uD83D\uDDF3 Продолжить после голосования', callback_data: 'posle_golosa_' + kod }]);
        }
        if (igra._pick_first_faza && !igra.tekushchiy_nomer) {
            knopki.push([{
        ...
        knopki.push([{ text: '\u26A0\uFE0F Выдать фол', callback_data: 'panel_foly_' + kod }]);
        knopki.push([knopkaImmuniteta(kod)]);
        knopki.push([{ text: '\uD83D\uDC80 Отметить выбывшего…', callback_data: 'panel_vybyl_' + kod }]);
    ...
    // ===== ПАНЕЛЬ ФОЛОВ =====
    ...
    else if (data.startsWith('mirny_podtv_')) {
        const kod = data.replace('mirny_podtv_', '');
        const igra = igry[kod];
        if (!igra) return;
        delete sostoyanie[telegram_id];
        bot.answerCallbackQuery(query.id);
        ...
        const kod = data.replace('mirny_edit_', '');
        const igra = igry[kod];
        if (!igra) return;
        sostoyanie[telegram_id] = 'noch_mirnye_' + kod;
        bot.answerCallbackQuery(query.id);
        bot.editMessageText(
            tekstVvodaSpiskaMirnyh(igra, kod) +
            '\n\n_Редактирование: отметь мирных по одному, введи списком или вернись к ночи знакомства._',
            {
                chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
                reply_markup: knopkiMirnyhVvoda(igra, kod, 'edit')
            }
        ...
        if (!igra) return;
        sostoyanie[telegram_id] = 'noch_mirnye_' + kod;
        bot.answerCallbackQuery(query.id, { text: 'Отправь список ников' });
        bot.sendMessage(chatId,
            '\u270D\uFE0F Отправь список мирных через запятую или каждый с новой строки.\n\n' +
            'Пример: `Аня, Оля, Катя`\n\n' +
            'Осталось мирных: *' + mirnyeOstalosVnesti(igra) + '*',
            { parse_mode: 'Markdown', reply_markup: knopkiMirnyhVvoda(igra, kod, 'edit') }
        );
    }
        ...
        // Таймер речи не стопаем — иначе «всё сбивается»
        otkrytOverlayTaymera(igra, 'foly');
    ...
    else if (data.startsWith('panel_vybyl_')) {
        const kod = data.replace('panel_vybyl_', '');
        const igra = igry[kod];
        if (!igra) return;
        const knopki = igra.igroki
            .filter(i => i.status === 'v_igre')
        ...
        knopki.push([{ text: '\u2B05\uFE0F Назад', callback_data: 'panel_' + kod }]);
        bot.editMessageText('\uD83D\uDC80 *Отметить выбывшего* — Игра \u2116' + kod + '\n\n_Ручная пометка вне голосования. После голосования дня 1 лучше идти через итог → лучший ход → ночь._', {
            chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
            reply_markup: { inline_keyboard: knopki }
        });
    }

    else if (data.startsWith('posle_golosa_')) {
        const kod = data.replace('posle_golosa_', '');
        const igra = igry[kod];
        if (!igra || !igra._posle_golosa) {
            bot.answerCallbackQuery(query.id, { text: 'Шаг уже пройден', show_alert: true });
            return;
        }
        bot.answerCallbackQuery(query.id);
        const st = igra._posle_golosa;
        await pokazatPosleGolosovaniya(chatId, messageId, kod, st.nomer, {
            shahidEffect: st.shahid,
            logMsg: st.log
        });
    }

    // ===== ПРОДОЛЖИТЬ ПОСЛЕ «ВОЗМОЖНОГО ИТОГА» =====
    else if (data.startsWith('prodolzhit_igru_')) {
        const kod = data.replace('prodolzhit_igru_', '');
        const igra = igry[kod];
        if (!igra) {
            await soobshitPosleKnopki(chatId, '❌ Игра не найдена. Открой «Мои игры».');
            return;
        }
        if (igra._posle_golosa) {
            const st = igra._posle_golosa;
        ...
        if (igra.faza === 'noch') {
            await pokazat_noch_panel(chatId, messageId, kod, '▶️ Игра продолжается — ночь.');
            return;
        }
        await bot.sendMessage(chatId,
            '▶️ *Игра продолжается.*\n\nФаза: *' + (igra.faza || '?') + '* · день ' + (igra.den || 1),
            {
                parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [
                    [{ text: '🎮 Панель игры', callback_data: 'panel_' + kod }],
                    [{ text: '\uD83C\uDF19 К ночи', callback_data: 'faza_noch_' + kod }],
                    [{ text: knopkaKtoNachinaet('den', igra.den || 1), callback_data: 'faza_den_' + kod }]
                ] }
            }
        ).catch(() => {});
    }
    ...
    // ===== ОТМЕТИТЬ ВЫБЫВШЕГО =====
        ...
        tekst2 += '\uD83D\uDC80 \u2116' + nomer + ' ' + igrok.name + ' отмечен выбывшим.\n';
        if (shahid_effect_ruchn) tekst2 += shahid_effect_ruchn + '\n';
        tekst2 += '\nВыбери следующий шаг процесса:';
        const knopki2 = [];
        if (igra.faza === 'golosovanie') knopki2.push([{ text: '\uD83D\uDDF3 К голосованию', callback_data: 'faza_golosovanie_' + kod }]);
        if (igra.faza === 'opravdanie') knopki2.push([{ text: '\u2B05\uFE0F К оправданию', callback_data: 'faza_opravdanie_' + kod }]);
        if (fazaKRuchiRechi(igra.faza) && igra.tekushchiy_nomer && !igra._krug_zavershen) {
            knopki2.push([{ text: '⏱ К таймеру', callback_data: 'timer_back_' + kod }]);
        }
        knopki2.push([{ text: '\uD83C\uDF19 К ночи', callback_data: 'faza_noch_' + kod }]);
        knopki2.push([{ text: '\uD83D\uDCCB Состав', callback_data: 'panel_' + kod }]);
    ...
    // ===== ВЫДАТЬ ФОЛ =====
        ...
        const knopki3 = igra.igroki
            .filter(i => i.status === 'v_igre')
            .map(i => [{ text: '\u26A0\uFE0F \u2116' + i.nomer + ' ' + i.name + ' (' + i.foly + '/' + maxFolyIgry(igra) + ')', callback_data: 'fol_' + kod + '_' + i.nomer }]);
        knopki3.push([{ text: '\u2B05\uFE0F Назад', callback_data: knopkaNazadKProcessu(igra, kod) }]);

        await sohranit_igru(kod);
        // Во время речи сразу возвращаемся к панели с таймером (таймер не останавливали)
        if (fazaKRuchiRechi(igra.faza) && igra.tekushchiy_nomer && !igra._krug_zavershen) {
            const cur = igra.igroki.find(i => i.nomer === igra.tekushchiy_nomer);
            ...
            await vernutPanelTaymera(igra, kod, chatId, messageId, {
                restartTimer: !(igra.taymer_aktiven && igra._interval),
                suffix: '\n\n\u26A0\uFE0F Фол ' + igrok.foly + '/' + max_foly + ' — *' + igrok.name + '*' +
                    (shahid_effect_fol ? '\n' + shahid_effect_fol : '')
            });
            return;
        }
    ...
    // ===== ИТОГ ИГРЫ =====
        ...
        const knopkiItoga = await knopkiPosleItogaIgry(kod, igra.klub_id, { druzya: druzyaRezhim });
        bot.editMessageText(svodka, {
            chat_id: chatId, message_id: messageId, parse_mode: 'Markdown',
            reply_markup: knopkiItoga
        });
    }
    ...
    // ===== ТАЙМЕР: личное время 40-60 секунд =====
        ...
        if (igra.tekushchiy_nomer && !igra._krug_zavershen && (igra.faza === 'den' || igra.faza === 'opravdanie')) {
            igra.taymer_sekundy = sek;
            zapustitTaymer(chatId, messageId, kod, sek);
            return;
        }
    ...
    else if (data.startsWith('set_opravdanie_')) {
        const parts = data.replace('set_opravdanie_', '').split('_');
        const kod = parts[0];
        const sek = Math.min(60, Math.max(10, parseInt(parts[1], 10) || 30));
        const igra = igry[kod];
        if (!igra) return;
        igra._nastroyki = igra._nastroyki || {};
        igra._nastroyki.opravdanie_sek = sek;
        bot.answerCallbackQuery(query.id, { text: 'Оправдание: ' + sek + ' сек' });
        if (igra.tekushchiy_nomer && !igra._krug_zavershen && igra.faza === 'opravdanie') {
            igra.taymer_sekundy = sek;
            zapustitTaymer(chatId, messageId, kod, sek);
            return;
        }
    ...
    else if (data.startsWith('restart_taymer_')) {
        const kod = data.replace('restart_taymer_', '');
        const igra = igry[kod];
        if (!igra) return;
        if (!igra.tekushchiy_nomer || igra._krug_zavershen || !fazaKRuchiRechi(igra.faza)) {
            bot.answerCallbackQuery(query.id, { text: 'Сейчас некому засекать время', show_alert: true });
            return;
        }
        const sek = sekundyTaymeraRechi(igra);
        igra.taymer_sekundy = sek;
        bot.answerCallbackQuery(query.id, { text: '▶️ ' + sek + ' сек' });
        zapustitTaymer(chatId, messageId, kod, sek);
    }
    ...
    // ===== ТАЙМЕР: ПАС =====
        ...
        // Успешный Пас: панель уже обновлена, лишние сообщения в чат не шлём
    }
    ...
    // ===== ВЫСТАВИТЬ НА ГОЛОСОВАНИЕ ПО НИКУ (во время минуты) =====
        ...
        if (!igra.tekushchiy_nomer) {
            bot.answerCallbackQuery(query.id, { text: 'Сейчас никто не говорит — добавь через «Добавить пропущенного»', show_alert: true });
            return;
        }
        if (uzheVystavilTekushchiyGovoryashchiy(igra)) {
        ...
        // Таймер НЕ останавливаем — речь идёт, пока ведущий выбирает кого выставить
        igra._taymer_ui_mode = 'picker';
        igra._picker_type = 'vystav';
        igra._vystav_ot_nomer = igra.tekushchiy_nomer;
        sostoyanie[telegram_id] = 'vystav_golos_' + kod;
        bot.answerCallbackQuery(query.id, { text: 'Выбери игрока (таймер идёт)' });
        const { chat_id, message_id: msgId } = idSoobshcheniyaTaymera(igra, chatId, messageId);
        await bot.editMessageText(tekstVystavleniyaNaGolos(igra, kod), {
            chat_id, message_id: msgId, parse_mode: 'Markdown',
            reply_markup: knopkiVystavleniyaNaGolos(igra, kod)
        });
    }
        ...
        if (igra.faza === 'opravdanie' && (!igra.tekushchiy_nomer || igra._krug_zavershen)) {
            await zavershitKrugRechi(chatId, messageId, kod);
            return;
        }
        // Не перезапускаем таймер, если он уже идёт
        await vernutPanelTaymera(igra, kod, chatId, messageId, { restartTimer: !(igra.taymer_aktiven && igra._interval) });
    }
    ...
    // ===== ВЫСТАВИТЬ НА ГОЛОСОВАНИЕ =====
        ...
        const igra = igry[kod];
        if (!igra) return;
        // Во время речи таймер не стопаем — только открываем список
        if (!(fazaKRuchiRechi(igra.faza) && igra.tekushchiy_nomer && igra.taymer_aktiven)) {
            stopTimer(kod);
        }
        igra._taymer_ui_mode = 'picker';
        igra._picker_type = 'nominacii';
        bot.answerCallbackQuery(query.id);
        const { chat_id, message_id: msgId } = idSoobshcheniyaTaymera(igra, chatId, messageId);
        ...
        // Таймер речи не останавливаем — дописываем пропущенного «на лету»
        if (!(fazaKRuchiRechi(igra.faza) && igra.tekushchiy_nomer && igra.taymer_aktiven)) {
            stopTimer(kod);
        }
        igra._taymer_ui_mode = 'picker';
    ...
    // ===== ФАЗА: ОПРАВДАНИЕ =====
        ...
        if (!igra.naznacheny_golos || igra.naznacheny_golos.length === 0) {
            await soobshitPosleKnopki(chatId, '❌ Никто не выставлен — сначала номинации в круге речи.');
            return;
        }
        // Назад из голосования / повторный вход — не рестартим круг с нуля
        if (igra.faza === 'golosovanie' || (igra.faza === 'opravdanie' && igra._krug_zavershen)) {
            stopTimer(kod);
            igra.faza = 'opravdanie';
            igra.tekushchiy_nomer = null;
            igra._krug_zavershen = true;
        ...
        if (!ok) {
            await soobshitPosleKnopki(chatId, '❌ Не удалось начать оправдание. Проверь список выставленных или открой панель.');
        }
    }
    ...
    // ===== ФАЗА: ГОЛОСОВАНИЕ =====
        ...
        stopTimer(kod);
        const uzheGolosovanie = igra.faza === 'golosovanie';
        igra.faza = 'golosovanie';
        igra.tekushchiy_nomer = null;
        // «Назад к голосованию» не должен обнулять уже внесённые голоса
        if (!uzheGolosovanie) {
            igra.golosa_dnya = {};
            oznachitImmunitetVGolosovanii(igra);
        }
        ...
        if (naznacheny_v.length === 0) {
            await soobshitPosleKnopki(chatId, '❌ Некого выводить на голосование — сначала выставь игроков.');
            return;
        }
        await sohranit_igru(kod);
        const textG = tekstGolosovaniyaSPodschetom(igra, kod);
        const optsG = {
            parse_mode: 'Markdown',
            reply_markup: { inline_keyboard: knopkiGolosovaniyaSPodschetom(igra, kod) }
        };
        try {
            await bot.editMessageText(textG, { chat_id: chatId, message_id: messageId, ...optsG });
        } catch (e) {
            console.error('[faza_golosovanie_]', kod, e?.message || e);
            await bot.sendMessage(chatId, textG, optsG).catch(() => {});
        }
    }
    ...
    // ===== ГОЛОСОВАНИЕ: внести количество голосов =====
        ...
        if (!igrok_gc) return;
        if (estImmunitetOtGolosovaniya(igrok_gc, igra)) {
            await soobshitPosleKnopki(chatId, '🛡 У игрока иммунитет — голоса за него не вводят.');
            return;
        }
    ...
    // ===== ГОЛОСОВАНИЕ: автоматический итог =====
        ...
        if (neVneseny.length > 0) {
            await soobshitPosleKnopki(chatId,
                '⚠️ Внеси голоса за всех, кроме последнего — он посчитается сам.');
            return;
        }
        if (!naznacheny.length) {
            await soobshitPosleKnopki(chatId, '⚠️ Все выставленные с иммунитетом — некого голосовать.');
            return;
        }
        const max = Math.max(...naznacheny.map(i => golosa[i.nomer] || 0));
        const lidery = naznacheny.filter(i => (golosa[i.nomer] || 0) === max);
        ...
        await otmetitPervuyuSmertDlyaImmuniteta(igra, igrok_gv, 'golos1');
        ...
        igra.peregolosovanie_finalisty = [];
        await sohranit_igru(kod);
        await pokazatPosleGolosovaniya(chatId, messageId, kod, igrok_gv.nomer, {
            shahidEffect: shahid_effect_gv,
            logMsg: '\uD83D\uDC80 Голосование: \u2116' + igrok_gv.nomer + ' выбыл'
        });
    }
    ...
    // ===== ПЕРЕГОЛОСОВАНИЕ: все спорные покидают стол =====
        ...
        if (finalisty[0]) await otmetitPervuyuSmertDlyaImmuniteta(igra, finalisty[0], 'golos1');
        if (effect_text) t += '\n' + effect_text;
    ...
    // ===== ФАЗА: НОЧЬ (переход) =====
        ...
        igra._noch_last_ok = step.tip === 'eskort'
            ? 'Эскортница: остальные выстрелы — пропуск'
            : (step.label + ' — пропуск');
        await sohranit_igru(kod);
        bot.answerCallbackQuery(query.id, { text: 'Пропуск' });
        igra._noch_guided_idx = idxPervogoNezavershennogoShaga(igra, 0);
        await pokazatShagNochiGuided(chatId, messageId, kod);
    }
        ...
        const igra = igry[kod];
        if (!igra) return;
        igra._noch_guided_idx = idxPervogoNezavershennogoShaga(igra, (igra._noch_guided_idx || 0) + 1);
        await sohranit_igru(kod);
    ...
    // ===== НОЧЬ: к дню после потерянных кнопок итогов =====
        ...
        if (!igra._noch_itogi_primeneny && igra.faza === 'noch') {
            const emptyActs = Object.keys(igra.noch_deystviya || {}).length === 0;
            const estUbitye = (igra.igroki || []).some(i => i.status !== 'v_igre');
            if (!((igra.den || 1) === 1 && emptyActs && estUbitye)) {
                await soobshitPosleKnopki(chatId,
    ...
    // ===== НОЧЬ: подтвердить итоги =====
    else if (data.startsWith('noch_itog_ok_')) {
        const kod = data.replace('noch_itog_ok_', '');
        const igra = igry[kod];
        if (!igra) return;
        const d = igra.noch_deystviya || {};
    ...
    else if (data.startsWith('noch_itog_edit_')) {
        const kod = data.replace('noch_itog_edit_', '');
        const igra = igry[kod];
        if (!igra) return;
        bot.answerCallbackQuery(query.id, { text: 'Исправь действия' });
        igra._noch_guided_idx = 0;
        await nachatNochGuided(chatId, messageId, kod);
    }

    // ===== НОЧЬ: черновик итогов (ещё не применяем) =====
        ...
        if (ubityPodryvnik && !d.podryvnik_zabiraet) {
            ...
            if (kandidatyPodryvnika.length > 0) {
                ...
                knopki_podryv.push([{ text: '\u2B05\uFE0F Назад', callback_data: 'noch_panel_' + kod }]);
                await pokazatHostPanel(igra, chatId, messageId,
                    '\uD83D\uDCA5 *Подрывника убила мафия!*\n\n' +
                    '\u2116' + ubityPodryvnik.nomer + ' *' + ubityPodryvnik.name + '* выбирает, кого забрать с собой:',
                    knopki_podryv);
                return;
        ...
        const t = tekstChernovikaItogovNochi(igra);
        const knopki = [
            [{ text: '\u2705 Применить итоги', callback_data: 'noch_itog_ok_' + kod }],
            [{ text: '\u270F\uFE0F Исправить действия', callback_data: 'noch_itog_edit_' + kod }],
            [{ text: '\uD83D\uDCCB Классическая панель', callback_data: 'noch_panel_' + kod }]
        ];
        await pokazatHostPanel(igra, chatId, messageId, t, knopki, { vnizu: true });
    }
    ...
    // ===== СОБСТВЕННИК: снять роль ведущего =====
        ...
        const shag = parseInt(m[1], 10);
        const idx = parseInt(m[2], 10);
        // Защита от нажатия на кнопки уже пройденного шага (старое сообщение в чате)
        if (shag !== (d.anketa_shag ?? 0)) {
            bot.answerCallbackQuery(query.id, { text: 'Этот шаг уже пройден' });
            return;
        }
        const pole = klubAnketa.SHAGI[shag];
        d.otvety = d.otvety || {};
